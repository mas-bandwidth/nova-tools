...
// internal/ci's TestEverySubVerbFlagSetAnswersHelp states in source: `nova-wake
...
func TestEverySubVerbAnswersHelp(t *testing.T) {
	for _, verb := range []string{
		...
		for _, spelling := range []string{"--help", "-h"} {
			...
			code := runWith(args, &out, &errs, wake.Real{})
			assertSubVerbHelp(t, "nova-wake", verb, spelling, code, out.String(), errs.String())
		}
	}
}
