...
func TestReadPostVerbMovesThePrimary(t *testing.T) {
	...
	out := post(1, "SCORE who=emma head="+head(1)+" score=9/10 gates=ci:green,base:ok,scope:ok")
	...
	r2 := reading("v2", 2)
	line := "SCORE who=emma head=" + head(2) + " score=6/10 gates=ci:green,base:ok,scope:ok: no failing test"
	out = post(2, line)
