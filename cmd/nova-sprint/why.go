...
func init() {
	...
	register(Verb{
		Name:    "land",
		Summary: "land --repo <r> --stream <s> | status|flaky|stream|merge|pr|run|offer|list ...: the whole stream landing (build, ci, merge on green), lander status, flaky store, its steps (land stream/merge, land status --repo), one PR merged by REST once its webhook check state in Redis is green (land pr <n>, #4311), and the fenced stream-PR lander (land run/offer/list, #2942)",
		Run:     runLand,
	})
}
