...
func init() {
	register(Verb{
		Name:    "card",
		Summary: "push, release, stop, show, launched, beat, and end one card attempt; fsck and ls --unplaced the card model",
		Run:     runCard,
	})
}
...
func runCard(ctx context.Context, args []string, stdout, stderr io.Writer) int {
	...
	if len(args) == 0 {
		return cardUsage(stderr, "", "wants push, release, stop, show, launched, beat, or end")
	}
	sub := args[0]
	want, ok := cardWant(sub)
	if !ok {
		return cardUsage(stderr, "unknown verb "+sub, "wants push, release, stop, show, launched, beat, or end")
	}
