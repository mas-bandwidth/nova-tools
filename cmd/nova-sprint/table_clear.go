...
func cmdTableClear(args []string, stdout, stderr io.Writer) int {
	...
	if err := fs.Parse(args); err != nil {
		return refuse(stderr, "table clear", err.Error())
	}
	actor := seatActor()
	by := &actor
	if fs.NArg() > 0 {
		return refuse(stderr, "table clear", "takes flags, not positional arguments")
	}
	addr := taskAddr(*redisAddr)
	if addr == "" || *checkpoint == "" {
		return refuse(stderr, "table clear", "needs --checkpoint <file> (the record written before anything moves) and --redis <addr> (or NOVA_SPRINT_REDIS / NOVA_REDIS_ADDR)")
	}
	who := *by
	if who == "" {
	...
	if err := writeAtomic(*checkpoint, plan.Checkpoint()); err != nil {
		return refuse(stderr, "table clear", "checkpoint: "+err.Error()+"; nothing was cleared")
	}
