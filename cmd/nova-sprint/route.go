...
func init() {
	register(Verb{
		Name:    "route",
		Summary: "run ok-to-friend, report-to-read, pr-to-read, hold-to-fix (and, once built, classify) for one sprint in one process under lease:route:<S>; a second instance exits 1 REFUSED",
		Run:     runRoute,
	})
}
