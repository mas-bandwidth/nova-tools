...
// cardLine is a task card verb's receipt: the TASK line (a push carries its
// order fields) and, after a move that can change a stream's order (#4322
// fix round), the stream's ORDER line.
var cardLine = regexp.MustCompile(`^TASK (\w+) id=(\S+) from=(\S+) to=(\S+) (?:order=\S+(?: order_rt=\d+(?: order_stale=\d+)? order_ms=[0-9.]+)? )?ms=\d+\n` +
	`(?:ORDER stream="[^"]*" order=\d+ order_rt=\d+(?: order_stale=\d+)? order_ms=[0-9.]+\n)?$`)
