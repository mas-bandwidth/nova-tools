...
// The token ledger (docs/SPEC-STATE.md `token_ledger`, #2201): `ledger` writes each folded
// day to the table as the day files' index, and `report --postgres` is the monthly report
// as one GROUP BY over it. The fold is untouched and the day TSVs stay the record; the
// table is what a month query reads instead of every file.

// openLedger is the store seam: the real path opens Postgres and migrates; the unit tests
// put record.FakeStore behind it, and RECORD_TEST_PG runs them against a live server.
var openLedger = func(dsn string) (record.LedgerStore, error) {
	s, err := record.OpenPostgres(dsn)
	if err != nil {
		return nil, err
	}
...
const wantsPostgres = "the Postgres DSN whose token_ledger table this reads or writes"

// ledgerEntries turns one day file into its token_ledger rows: the card is the row's unit
...
// cmdLedger indexes the day files of one day or one month into token_ledger. Each day is
...
func cmdLedger(args []string, stdout, stderr io.Writer) int {
	...
	r.required("postgres", *dsn, wantsPostgres)
	switch {
	case *day == "" && *month == "":
	...
	store, err := openLedger(*dsn)
	if err != nil {
		fmt.Fprintf(stderr, "LEDGER FAILED store=postgres err=%s\n", oneline.Err(err))
		return 1
	}
	defer store.Close()
...
// cmdReportStore is `report --postgres`: the month's token_ledger grouped by model, repo,
// day, or the (day, model, repo) tuple, every one of the five types apart and a dash where
// no row reported a type.
func cmdReportStore(dsn, month, by string, max int, stdout, stderr io.Writer) int {
	...
	store, err := openLedger(dsn)
	if err != nil {
		fmt.Fprintf(stderr, "REPORT FAILED store=postgres err=%s\n", oneline.Err(err))
		return 1
	}
	defer store.Close()
	...
	fmt.Fprintf(stdout, "REPORT OK month=%s source=token_ledger groups=%d rows=%d\n", oneline.Field(month), len(totals), rows)
	return 0
}
