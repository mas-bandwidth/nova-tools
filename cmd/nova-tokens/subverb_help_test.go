...
// internal/ci's TestEverySubVerbFlagSetAnswersHelp states in source: `nova-tokens
...
func TestEverySubVerbAnswersHelp(t *testing.T) {
	for _, verb := range []string{
		...
		for _, spelling := range []string{"--help", "-h"} {
			...
			code := run(args, &out, &errs, time.Time{})
			assertSubVerbHelp(t, "nova-tokens", verb, spelling, code, out.String(), errs.String())
		}
	}
}
