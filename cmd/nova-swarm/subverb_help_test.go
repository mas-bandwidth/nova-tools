...
// internal/ci's TestEverySubVerbFlagSetAnswersHelp states in source: `nova-swarm
...
func TestEverySubVerbAnswersHelp(t *testing.T) {
	for _, verb := range []string{
		...
		for _, spelling := range []string{"--help", "-h"} {
			...
			code := run(args, strings.NewReader(""), &out, &errs, time.Time{})
			assertSubVerbHelp(t, "nova-swarm", verb, spelling, code, out.String(), errs.String())
		}
	}
}
