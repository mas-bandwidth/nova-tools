...
type nativeRunResult struct {
	...
	harness      string  // ok | silent: silent when the harness left neither harness.log nor a RESULT.md
}
...
// when the harness process left NO record of itself -- not a byte in `<job>/harness.log`, and
// no `RESULT.md` anywhere the gather looks for one -- and `ok` otherwise (issue #591).
//
...
//
// THE HARNESS LOG IS THE RUN'S OWN CAPTURE (issue #608): this process pins the child's stdout
// and stderr to `<job>/harness.log` as well as `<slot>/native.log`, walled or not, so a
// harness that SAID anything at all is `ok` -- there is evidence to read -- and `silent` is
// the run that produced neither a word nor a result.
//
...
func harnessState(jobDir string) string {
	if wroteBytes(filepath.Join(jobDir, "harness.log")) {
		return "ok"
	}
...
// wroteBytes says whether a path is a regular file holding at least one byte: the one test
// harnessState applies to each record, so an empty file is nothing written wherever it sits.
