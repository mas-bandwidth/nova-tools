...
func nativeRun(cfg nativeRunConfig, errOut io.Writer) (nativeRunResult, int) {
	...
	for attempt := 1; ; attempt++ {
		...
		// THIS LAUNCH'S BYTES, FROM THE PARENT TEE (issue #1892), never a re-read of the
		// card-writable <job>/harness-output.log.
		tail := classified.Bytes()[before:]
		...
		// An overflowed parent tee is unverifiable: never retry on a truncated prefix.
		if classified.Hit() {
			break
		}
