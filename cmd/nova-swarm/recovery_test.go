...
func TestADeadDispatcherIsRecoveredOrQuarantined(t *testing.T) {
	...
	write(t, filepath.Join(task2Dir, "RESULT.md"), "# a recovered job\n\n## Head\nfindings: 0\nnotes read: 0\nrepo: o/n\nrev: abc\nit finished before its dispatcher died.\n\n## Findings\n- none\n")
	...
	write(t, filepath.Join(task6Dir, "RESULT.md"), "# a recovered job\n\n## Head\nfindings: 0\nnotes read: 0\nrepo: o/n\nrev: abc\nit finished before its dispatcher died.\n\n## Findings\n- none\n")
	...
	mustContain(t, "stdout", stdout, "RUN RECLAIM slot=2 id="+task2ID+" end=done dest=done usage=")
	...
	mustContain(t, "stdout", stdout, "RUN RECLAIM slot=6 id="+task6ID+" end=unknown dest=failed usage=")
