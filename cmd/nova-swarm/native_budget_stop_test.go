...
func TestNativeCardBudgetStopsAndPrintsThePromptDefect(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// asserts is that the defect line comes after it and is written into no file.
			lines := strings.Split(strings.TrimRight(out, "\n"), "\n")
			okAt, defectAt := -1, -1
			for i, l := range lines {
				if strings.HasPrefix(l, "NATIVE OK ") || strings.HasPrefix(l, "NATIVE INCOMPLETE ") {
					okAt = i
				}
