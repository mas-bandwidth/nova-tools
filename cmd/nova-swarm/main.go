...
func cmdNative(args []string, stdout, stderr io.Writer) int {
	...
	// AN UNREAD DENIAL IS NEVER AN OK (issue #1465; Stella's HOLD on #1478; issue #1892).
	// This is the ONE refusal that lands AFTER the spend, and it is a refusal rather than a
	// token on the OK line on purpose: the run of #1465 carried `rc=0 sandbox=landlock
	// harness=ok` over a card whose shell had been denied the toolchain, and a coordinator
	// reading dispositions and not prose shipped a commit nobody had compiled. There is no
	// OK line here at all. The bytes classified are the parent's tee, never a re-read of
	// the job file the child can replace.
	//
