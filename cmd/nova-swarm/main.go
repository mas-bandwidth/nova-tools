...
func cmdBatch(args []string, stdout, stderr io.Writer, now time.Time) int {
	...
	if *cards != "" {
		return cmdBatchGather(f, *id, *cards, *deadline, *runner, *root, *idle, *benches, *bench, *noWall, stdout, stderr)
	}
	f.want(*pool, "pool", "the directory that holds this pool's tasks")
...
// bounded packet.
func cmdBatchGather(f *flags, id, cards, deadline, runner, root string, idle int, benches, bench string, noWall bool, stdout, stderr io.Writer) int {
	...
	return swarm.Batch(swarm.BatchInput{
		...
		Benches: benches, Bench: bench, NoWall: noWall,
