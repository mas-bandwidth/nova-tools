...
func cmdNative(args []string, stdout, stderr io.Writer) int {
	...
	fmt.Fprintf(stdout, "NATIVE OK label=%s job=%s rc=%d wall=%.2fs sandbox=%s card_sha256=%s binary_sha256=%s config=%s harness=%s%s\n",
		oneline.Field(cfg.label), oneline.Field(res.job), res.rc, res.wallSeconds, oneline.Field(res.wall), oneline.Field(res.cardSHA256), oneline.Field(res.binarySHA256), oneline.Field(dash(res.configSHA)), oneline.Field(orElse(res.harness, "silent")), usageSuffix(res.usageReason, res.usageState))
	if res.rc != 0 {
		if res.rc > 0 {
			return res.rc
		}
