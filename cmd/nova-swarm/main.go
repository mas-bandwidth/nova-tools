...
usage:
  ...
  nova-swarm batch     --id <id> --cards <file> --deadline <seconds> --runner <cmd> --root <dir> [--idle <seconds>] [--then <command>]
...
func cmdBatch(args []string, stdout, stderr io.Writer, now time.Time) int {
	...
	if *cards != "" {
		return cmdBatchGather(f, *id, *cards, *deadline, *runner, *root, *idle, *then, stdout, stderr)
	}
	f.want(*pool, "pool", "the directory that holds this pool's tasks")
...
// bounded packet.
func cmdBatchGather(f *flags, id, cards, deadline, runner, root string, idle int, then string, stdout, stderr io.Writer) int {
	...
	return swarm.Batch(swarm.BatchInput{
		...
		Cards: cards, Root: root, Runner: runner, Then: then,
