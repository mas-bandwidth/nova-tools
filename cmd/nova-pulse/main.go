...
nova-pulse deal    --undealt <dir> [--priority <dir>] --ready-root <dir> --bench <name>... --redis <addr> --route-table <file> [--max-load-per-core 1.5] [--reading-debt <n> --debt-cap <n>] [--launched <dir>]... [--repo <path> --base dev --results <dir>] [--dry-run]
...
func run(args []string, stdout, stderr io.Writer, now time.Time) int {
	...
	case "deal":
		return cmdDeal(rest, stdout, stderr)
	case "cut":
		if hasKindFlag(rest) {
