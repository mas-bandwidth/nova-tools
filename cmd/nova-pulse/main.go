...
example:
  nova-pulse cut --pool ./pool.tsv --templates ./templates --out ./cards --root .

cut reads a pool.tsv of source, id, kind, title, template and writes one card per
candidate from its typed template into --out, plus a cards.tsv naming the model by
kind; it makes no model call either. docs/TESTS.md carries the transcript it prints.
`
...
func cmdCut(args []string, stdout, stderr io.Writer) int {
	...
	if *max < 0 {
		f.add(fmt.Sprintf("--max is 0 or more, got %d; 0 already means all, so a negative ceiling is a typo with two readings", *max))
	}
	if f.refused(stderr) {
		return 2
	}
