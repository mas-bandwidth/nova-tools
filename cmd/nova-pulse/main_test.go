...
// Today pool writes the kind into pool.tsv field 1, so every card clones github.com/issues.git
// (the dogfood probe's quoted line); pool must write the locator, and cut renders <source>
// from it. This test runs the pool then the cut command from the issue and asserts the
// expected STEP 1 line.
