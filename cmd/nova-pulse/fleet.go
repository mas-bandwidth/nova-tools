...
func cmdFleet(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		return refuse(stderr, " fleet", "a sub-verb is required (survey, suspend, wake, reboot, secrets, hygiene)")
	}
	...
	fmt.Fprintf(stderr, "nova-pulse fleet: unknown sub-verb %q (the sub-verbs are survey, suspend, wake, reboot, secrets, hygiene; run: nova-pulse help)\n", sub)
	return 2
}
