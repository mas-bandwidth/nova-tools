...
func cmdFleet(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		return refuse(stderr, " fleet", "a sub-verb is required (registry, add, survey, suspend, wake, reboot, secrets, standard, mirror, join, sleep, status)")
	}
	...
	case "status":
		return cmdFleetStatus(rest, stdout, stderr)
	}
	fmt.Fprintf(stderr, "nova-pulse fleet: unknown sub-verb %q (the sub-verbs are registry, add, survey, suspend, wake, reboot, secrets, standard, mirror, join, sleep, status; run: nova-pulse help)\n", sub)
	return 2
}
