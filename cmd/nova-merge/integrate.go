...
func integrateCI(in integrateRun, host merge.Host, pr int, head string, stdout, stderr io.Writer, deps Deps, start time.Time) int {
	...
	for {
		...
		state := checkState(checks.ForSHA(head), batchRequiredCheckDefault)
		if state != last {
			fmt.Fprintf(stdout, "INTEGRATE CI pr=%d head=%s check=%s state=%s t=%.1fs\n",
				pr, oneline.Field(merge.Short(head)), batchRequiredCheckDefault, oneline.Field(state), since(start))
			last = state
		}
		switch state {
		case "green":
		...
		if !deps.Now().Before(deadline) {
			fmt.Fprintf(stdout, "INTEGRATE CI pr=%d head=%s check=%s state=timeout waited=%s\n",
				pr, oneline.Field(merge.Short(head)), batchRequiredCheckDefault, oneline.Field(in.ciTimeout.String()))
			return integrateRefused(stderr, "ci", fmt.Sprintf(
				"%s was still %s after %s; the pull request stands and nothing was queued", batchRequiredCheckDefault, oneline.Field(state), oneline.Field(in.ciTimeout.String())), 0)
		}
...
func integrateCIRed(in integrateRun, pr int, stdout, stderr io.Writer, deps Deps) int {
	if deps.NewFailForge == nil {
		return integrateRefused(stderr, "ci", fmt.Sprintf("%s is red on pull request %d", batchRequiredCheckDefault, pr), 0)
	}
	forge := deps.NewFailForge(in.repo, in.timeout)
	if forge == nil {
		return integrateRefused(stderr, "ci", fmt.Sprintf("%s is red on pull request %d, and this build has no edge to read the run through", batchRequiredCheckDefault, pr), 0)
	}
	runID, report, err := ci.ReadFailedRun(forge, ci.RunSelector{PR: pr}, "")
	if err != nil {
		fmt.Fprintf(stdout, "INTEGRATE FAIL pr=%d test=unread reason=%q\n", pr, oneline.Err(err))
		return integrateRefused(stderr, "ci", fmt.Sprintf(
			"%s is red on pull request %d and the run could not be read to name the test: %s", batchRequiredCheckDefault, pr, oneline.Err(err)), 0)
	}
	if len(report.Failures) == 0 {
		...
		return integrateRefused(stderr, "ci", fmt.Sprintf(
			...
			batchRequiredCheckDefault, pr, runID, oneline.Escape(oneline.Cap(report.SummaryLine(), oneline.TailBytes))), 0)
	}
	...
	return integrateRefused(stderr, "ci", fmt.Sprintf(
		...
		batchRequiredCheckDefault, pr, len(report.Failures), oneline.Field(report.Failures[0].Test)), 0)
}
