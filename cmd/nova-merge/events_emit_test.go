...
func TestReactEmitsTheMessageItReactedTo(t *testing.T) {
	t.Parallel()
	// EDGE 17: --lane IS the door a green pull request goes through, and it is required.
	// The reactor enqueues into the lane's own queue.json, which is what `queue status`
	// reads, so the lane this lab builds is what the reaction is checked against.
	l := newLab(t)
	l.init("main")
	mr := miniredis.RunT(t)
	...
	logPath := filepath.Join(t.TempDir(), "nova-events-react.log")
	done := make(chan int, 1)
	errs := make(chan string, 1)
	go func() {
		exit, so, se := l.run("react", "--redis", mr.Addr(), "--lane", l.lane, "--once",
			"--deadline", "30", "--log", logPath, "--bench", "hulk")
		errs <- so + se
		done <- exit
	}()
	...
	for {
		...
		case code := <-done:
			if code != 0 {
				t.Fatalf("react --once exit = %d: %s", code, <-errs)
			}
			goto read
		case <-deadline:
...
read:
	if q := l.loadQueue(); !eqInts(q.Queued, []int{42}) {
		t.Fatalf("the lane's own order after react is %v, want [42]", q.Queued)
	}
