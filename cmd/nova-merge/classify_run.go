...
// This file is the `classify` verb: one typed decision about one FAILED merge-group run
// (flaky under load, the pull request's own change, or the environment). It lives beside
// classify.go, which is the packet entry's risk/scope classification (`packet --decide`):
// two different asks over two different states, so they are two files and share no names.
