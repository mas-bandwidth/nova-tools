...
func TestVersionLineShape(t *testing.T) {
	...
	if code := cmdVersion(nil, &out, &errOut, versionDeps()); code != 0 {
	...
	if len(fields) != 5 {
		t.Fatalf("want 5 fields, got %d: %q", len(fields), line)
	}
	if fields[0] != "nova-merge" {
	...
	// The fifth field is this binary's own answer to a different question -- which BYTES
	// are on disk, rule 16's check -- and it is kept, named, after the four every binary
	// in the set shares. Before this verb changed, the whole line was `nova-merge
	// <buildID>`: two tokens, with the file hash standing where every other binary puts
	// its build identity.
	if want := "build=aaaabbbbcccc"; fields[4] != want {
		t.Errorf("field 5: got %q, want %q", fields[4], want)
	}
}
...
func TestVersionLineHoldsWhateverTheStampContains(t *testing.T) {
	...
	version = "v1.2.3\nnova-merge v9.9.9 linux/amd64 go1.0 extra"
	...
	if fields := strings.Fields(strings.TrimSuffix(line, "\n")); len(fields) != 5 {
		t.Fatalf("want 5 fields whatever the stamp holds, got %d: %q", len(fields), line)
	}
...
func TestVersionRefusesFlagsAndArguments(t *testing.T) {
	t.Parallel()
	for _, args := range [][]string{{"--short"}, {"extra"}, {"--lane", "."}} {
		var out, errOut bytes.Buffer
		if code := cmdVersion(args, &out, &errOut, versionDeps()); code != 2 {
...
func TestVersionVerbIsReachableFromTheDispatch(t *testing.T) {
	...
	for _, verb := range []string{"version", "--version"} {
		var out, errOut bytes.Buffer
		if code := run([]string{verb}, &out, &errOut, versionDeps()); code != 0 {
			t.Errorf("%s: exit %d, want 0\nstderr: %s", verb, code, errOut.String())
			continue
		}
		if !strings.HasPrefix(out.String(), "nova-merge ") || strings.Count(out.String(), "\n") != 1 {
			t.Errorf("%s: not the version line: %q", verb, out.String())
		}
	}
}

// versionDeps is the whole of what this verb reaches outside itself: the id of the binary
// on disk, which comes through Deps like everything else here, so the test drives the
// verb without a binary to hash.
func versionDeps() Deps {
	return Deps{BuildID: func() string { return "aaaabbbbcccc" }}
}
