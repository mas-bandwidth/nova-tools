...
// internal/ci's TestEverySubVerbFlagSetAnswersHelp states in source: `nova-post
...
func TestEverySubVerbAnswersHelp(t *testing.T) {
	for _, verb := range []string{"draft", "show", "send"} {
		for _, spelling := range []string{"--help", "-h"} {
			var out, errs bytes.Buffer
			code := run([]string{verb, spelling}, &out, &errs)
			assertSubVerbHelp(t, "nova-post", verb, spelling, code, out.String(), errs.String())
		}
	}
}
