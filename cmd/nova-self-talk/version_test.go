...
// The SHAPE, asserted field by field. `nova-self-talk <identity> <goos>/<goarch> <go version>` is
...
//
//	nova-self-talk: unknown subcommand "version"; run: nova-self-talk help
//
// on stderr at exit 2.
...
func TestVersionVerbIsReachableFromTheDispatch(t *testing.T) {
	t.Parallel()
	// `version` is recognised as the WHOLE invocation, the way `help` is: this tool takes
	// files positionally, so it is a verb only when it stands alone.
	for _, verb := range []string{"version", "--version"} {
		var out, errOut bytes.Buffer
		if code := run([]string{verb}, &out, &errOut); code != 0 {
			t.Errorf("%s: exit %d, want 0\nstderr: %s", verb, code, errOut.String())
			continue
		}
		if !strings.HasPrefix(out.String(), "nova-self-talk ") || strings.Count(out.String(), "\n") != 1 {
			t.Errorf("%s: not the version line: %q", verb, out.String())
		}
	}
	// And a file actually named `version`, named beside another file, is still a FILE:
	// the verb took nothing away from the scan.
	dir := t.TempDir()
	for _, name := range []string{"version", "other.md"} {
		if err := os.WriteFile(filepath.Join(dir, name), []byte("# a file\n"), 0o644); err != nil {
			t.Fatal(err)
		}
	}
	var out, errOut bytes.Buffer
	if code := run([]string{filepath.Join(dir, "version"), filepath.Join(dir, "other.md")}, &out, &errOut); code != 0 {
		t.Fatalf("two named files: exit %d, want 0\nstderr: %s", code, errOut.String())
	}
	if strings.Contains(out.String(), runtime.Version()) {
		t.Errorf("a file named version was answered with the version line: %q", out.String())
	}
}
