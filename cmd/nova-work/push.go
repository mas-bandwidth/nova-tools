...
func cmdPush(args []string, stdout, stderr io.Writer, now func() time.Time) int {
	...
	if err := fs.Parse(args); err != nil {
		if err == flag.ErrHelp {
			return printVerbHelp(stderr, "push")
		}
	...
	if st := strings.TrimSpace(*stream); st != "" && !safepath.NameOK(st) {
		problems = append(problems, fmt.Sprintf("--stream %s is not [A-Za-z0-9._-]+, so it cannot name a stream directory on disk", oneline.Field(st)))
	}
