...
func TestPushRefusesUnsafeStream(t *testing.T) {
	...
	for _, badStream := range []string{"../../escaped", "stream/slash", "..", "-flaglike"} {
		code, _, errb := runPush(t, "--stream", badStream, "--lane", "red", "--card", path, "--dir", root)
		if code != 2 {
			t.Errorf("stream %q exit = %d, want 2; stderr=%s", badStream, code, errb)
		}
		if !strings.Contains(errb, "--stream") {
			t.Errorf("stream %q stderr %q does not name --stream", badStream, errb)
		}
	}
}

// TestPushDirectoryModeMultilineBodyRoundTrips: cards carry instructions, prompts, and context
...
func TestPushDirectoryModeMultilineBodyRoundTrips(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(multilineBody), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if card.ID != id {
		t.Errorf("got card ID %s, want %s", card.ID, id)
	}
	if card.Fields["body"] != multilineBody {
		t.Fatalf("body did not round-trip:\ngot:  %q\nwant: %q", card.Fields["body"], multilineBody)
	}
}
