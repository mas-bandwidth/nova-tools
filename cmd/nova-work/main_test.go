...
func TestHelpListsEveryVerbTheSwitchAccepts(t *testing.T) {
	verbs := switchVerbs(t)
	want := []string{"help", "query ready", "query remaining", "query size", "session start", "session status", "session stop", "version"}
...
func switchVerbs(t *testing.T) []string {
	...
	for _, c := range top.Body.List {
		...
		if names[0] == "session" || names[0] == "query" {
...
func TestUnusableInvocationsAreRefusedAtTwo(t *testing.T) {
	cases := []struct {
		...
		{"query without a subverb", []string{"query"}, "size, remaining or ready"},
		{"unknown query verb", []string{"query", "bogus"}, "unknown query verb"},
