...
func TestHelpListsEveryVerbTheSwitchAccepts(t *testing.T) {
	verbs := switchVerbs(t)
	want := []string{"clip", "dependencies", "help", "operation cancel", "operation list", "operation status", "operation wait", "plan", "ready", "session start", "session status", "session stop", "snapshot", "version"}
...
// first switch statement in run's body is the verb switch; the cases whose
// value is "session" or "operation" hold the sub-verb switches; alias
// spellings (--help, -h) share their case's first value and are not verbs of
// their own.
func switchVerbs(t *testing.T) []string {
	...
	for _, c := range top.Body.List {
		...
		if names[0] == "session" || names[0] == "operation" {
...
func TestUnusableInvocationsAreRefusedAtTwo(t *testing.T) {
	cases := []struct {
		...
		{"snapshot without --out", []string{"snapshot", "--session", "x"}, "--out is required"},
