...
func TestEveryFlagInTheSynopsisIsDefinedByItsVerb(t *testing.T) {
	...
	// fails that nobody notices. Nine LINES over eight verbs, and the flag count, as the
	...
	// can paste.
	if got := len(block); got != 9 {
		t.Errorf("the synopsis parsed to %d verb lines, want 9: %+v", got, block)
	}
	if checked < 30 {
