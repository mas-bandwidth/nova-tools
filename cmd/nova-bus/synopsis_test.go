...
func TestEveryFlagInTheSynopsisIsDefinedByItsVerb(t *testing.T) {
	...
	// fails that nobody notices. Twelve LINES over eleven verbs, and the flag count, as
	// the banner stands; a deliberate change to either moves these numbers in the same
	// commit -- it moved once here, for class K of #828, where `wake` and `receipts` are
	// two new verbs and two new lines, and the two flags added to lines already here,
	// `receipt --verdict` and `close --older-than`, are checked by the loop above.
