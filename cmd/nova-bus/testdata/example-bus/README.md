...
A four-note bus in the shape `nova-bus` leaves one in, so that a mind arriving
...
Ada's `OPEN` is worth opening first, because it is where the tool's one
performance property lives: each line is a note Ada has been shown and not
...
It is not a fixture with a trick in it. `nova-bus check --bus . --full` passes
...
The one thing it cannot show is git, which is the transport. A real bus is a
**private** repository whose ROOT is a directory shaped like this one — not a
subdirectory of a larger repository. `nova-bus` refuses a `--bus` that is not
its repository's root for every verb that reads git, because git reports changed
paths relative to the repository root, so a bus one directory down would report
...
bus. To try it, copy it out and give it a repository of its own:
