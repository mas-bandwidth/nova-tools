...
// internal/ci's TestEverySubVerbFlagSetAnswersHelp states in source: `nova-memory
...
func TestEverySubVerbAnswersHelp(t *testing.T) {
	for _, verb := range []string{
		...
		for _, spelling := range []string{"--help", "-h"} {
			...
			assertSubVerbHelp(t, "nova-memory", verb, spelling, code, out.String(), errs.String())
		}
	}
}
