...
// internal/ci's TestEverySubVerbFlagSetAnswersHelp states in source: `nova-review
...
func TestEverySubVerbAnswersHelp(t *testing.T) {
	for _, verb := range []string{
		...
		for _, spelling := range []string{"--help", "-h"} {
			...
			assertSubVerbHelp(t, "nova-review", verb, spelling, code, out.String(), errs.String())
		}
	}
}
