// nova-review version: which build is running.
//
// A review packet is evidence about a BUILD, and until this verb existed this binary
// could not say which one it was: `nova-review version` printed a hand-written "devel"
// that was wrong exactly at the commit after the release. So "we are all running the
// same nova-review" was a belief rather than a reading.
//
...
func cmdVersion(args []string, stdout, stderr io.Writer) int {
	if len(args) > 0 {
		fmt.Fprintf(stderr, "nova-review version: takes no flags and no arguments, got %d\n", len(args))
		return 2
	}
	fmt.Fprintln(stdout, buildinfo.Line("nova-review", version))
	return 0
}
