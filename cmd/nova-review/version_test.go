...
// The SHAPE, asserted field by field. `nova-review <identity> <goos>/<goarch> <go version>` is
...
//
// Before this verb existed the same invocation printed "nova-review devel" -- a
// hand-maintained floor that was wrong exactly at the commit after the release.
...
func TestVersionLineHoldsWhateverTheStampContains(t *testing.T) {
	...
	version = "v1.2.3\nnova-review v9.9.9 linux/amd64 go1.0 extra"
...
func TestVersionVerbIsReachableFromTheDispatch(t *testing.T) {
	...
	for _, verb := range []string{"version", "--version"} {
		...
		if !strings.HasPrefix(out.String(), "nova-review ") || strings.Count(out.String(), "\n") != 1 {
