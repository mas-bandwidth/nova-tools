...
// The grammar block in docs/SPEC-REVIEW.md lists one line per event for the
// verbs. The binary ships `packet`, `mutate`, `guard`, `version` and `help`; the other six
...
func TestGrammarStrikesTheUnbuiltVerbs(t *testing.T) {
	...
	shipped := map[string]bool{"PACKET": true, "MUTATE": true, "GUARD": true}
