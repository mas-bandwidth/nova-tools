...
// internal/ci's TestEverySubVerbFlagSetAnswersHelp states in source: `nova-board
...
func TestEverySubVerbAnswersHelp(t *testing.T) {
	for _, verb := range []string{
		...
		for _, spelling := range []string{"--help", "-h"} {
			...
			code := run(args, &out, &errs, time.Time{}, bytes.NewReader(make([]byte, 64)))
			assertSubVerbHelp(t, "nova-board", verb, spelling, code, out.String(), errs.String())
		}
	}
}
