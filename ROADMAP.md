...
State read 2026-09-19 at `dev` [`4c793b55`](https://github.com/mas-bandwidth/nova-tools/commit/4c793b55a30160e5fe1ed45e25928f2c85dcfe5c).

### Release blockers

A blocker is an open defect where the tool gives wrong output, loses data, or a spend or admission control is missing. A release-gate line cannot go green while one is open. Sixteen were filed; fifteen are still open.

| Issue | Area | What it is | State |
|---|---|---|---|
| [#1607](https://github.com/mas-bandwidth/nova-tools/issues/1607) | merge | two `nova-merge` batch refusal tests fail in `t.TempDir` cleanup on ubuntu-latest, and `certification-ok` fails behind them | open; [PR #1608](https://github.com/mas-bandwidth/nova-tools/pull/1608) open, green, unread |
| [#1609](https://github.com/mas-bandwidth/nova-tools/issues/1609) | merge | `silenceRedis` writes a go-redis package global on every `react`: a data race under `-race`, one run in six | open; no PR |
| [#1572](https://github.com/mas-bandwidth/nova-tools/issues/1572) | merge | `batch` and `land` admit a member carrying an unlifted HOLD; a held head reached `dev` through a green gate | open; no PR |
| [#1545](https://github.com/mas-bandwidth/nova-tools/issues/1545) | swarm | no token, call or cost cap on the direct-provider route; `--deadline` is the only bound | open; spec landed ([#1566](https://github.com/mas-bandwidth/nova-tools/pull/1566) in integration [#1579](https://github.com/mas-bandwidth/nova-tools/pull/1579) at `06459eaa`), code in flight |
| [#1546](https://github.com/mas-bandwidth/nova-tools/issues/1546) | swarm | `native` takes no bench slot lease although the spec refuses a launch without one | open; [PR #1562](https://github.com/mas-bandwidth/nova-tools/pull/1562) landed in integration [#1601](https://github.com/mas-bandwidth/nova-tools/pull/1601) at `8e11a144` — the issue wants closing with its receipt |
| [#1465](https://github.com/mas-bandwidth/nova-tools/issues/1465) | swarm | `native` shares the Go caches but never puts the Go toolchain in the read set, so a Go card cannot run its own gate | open; [PR #1478](https://github.com/mas-bandwidth/nova-tools/pull/1478) open, conflicting |
| [#1463](https://github.com/mas-bandwidth/nova-tools/issues/1463) | swarm | the harness fence ignores `read_roots`, so a staged read is refused while the run still prints `NATIVE OK` | open; [PR #1478](https://github.com/mas-bandwidth/nova-tools/pull/1478) open, conflicting |
...
| [#1518](https://github.com/mas-bandwidth/nova-tools/issues/1518) | bus | the inbox and wait walk is bounded at 500 commits, so a line further behind never sees new mail | open; no PR |
...
| [#1512](https://github.com/mas-bandwidth/nova-tools/issues/1512) | pulse | `nova-pulse hygiene` still carries the three unsafe reaper rules the script dropped; this shape has eaten two certify trees | open; no PR |
| [#1504](https://github.com/mas-bandwidth/nova-tools/issues/1504) | ci | the `AGENTS.md` class-rule contract never runs on the pull request that breaks it | open; no PR |
...
| [#1466](https://github.com/mas-bandwidth/nova-tools/issues/1466) | check | the dogfood gate passes on an empty receipt set, so the release lane can go green on zero evidence | open; [PR #1558](https://github.com/mas-bandwidth/nova-tools/pull/1558) open, green, unread |
