...
(defun %verify-resolver-dir ()
  "A fresh 0700 directory for one case's resolver scripts, under this run's own
root (harness.lisp `test-temp-dir'), so two suites sharing a host can never build
the same path (nova-tools#1699)."
  (test-temp-dir "verify-resolver"))
...
(deftest "a-resolver-is-never-run-through-a-shell"
  ...
  (let* ((dir (%verify-resolver-dir))
         (witness (namestring (merge-pathnames "witness" dir)))
         (pwned (merge-pathnames "pwned" (test-run-root)))
         (pointer (format nil "commit:$(touch ~A);x" (namestring pwned)))
         (subject "a b;c"))
