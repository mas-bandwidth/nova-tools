...
in all .lisp files under DIRECTORY."
  (let ((names '()))
    (dolist (f (uiop:directory-files directory "*.lisp"))
      (with-open-file (s f :direction :input :if-does-not-exist nil)
        (when s
          (loop for line = (read-line s nil nil) while line
                when (and (search pattern line)
                          (let ((pos (search pattern line)))
                            (and pos (char= #\( (char line (+ pos (length pattern)))))))
                  do (let* ((start (+ pos (length pattern) 1))
                            (end (position #\Space line :start start))
                            (name (subseq line start (or end (length line)))))
