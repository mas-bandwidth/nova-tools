...
The kernel's view is what `%dep-submit` reads INSIDE the writer, so a fixture
that settles a need and then expects the counts on the DEP line installs the
view the same way a session would: from the tree, after the settle."
...
(deftest "a-dep-edit-survives-a-restart-and-its-no-effect-is-recorded" "docs/SPEC-WORK.md:4993,2982"
  ...
  (flet ((restart-of (k)
    ...
    ;; a FRESH request naming an edge that is already there: THE NO-EFFECT
    ;; RECEIPT (SPEC-WORK.md:2982-2986) -- an accepted typed `:structure`
    ;; event with a real id, changed=0, the revision advanced, nothing moved
    (let* ((k (dep-kernel))
           (history (length (state-history (kernel-state k))))
      ...
      (multiple-value-bind (okp line code)
        ...
        (check-equal 0 code "at exit 0")
        (ok (search "DEP OK id=" line) "as an accepted receipt with a real id: ~A" line)
        (ok (search "changed=0" line) "with changed=0: ~A" line))
      (check-equal (1+ history) (length (state-history (kernel-state k)))
                   "and it writes exactly one typed event")
      (ok (> (state-revision (kernel-state k)) rev)
          "and the event revision advanced")
      (check-equal :structure (getf (first (getf (car (last (state-history (kernel-state k))))
                                                  :events))
                                   :kind)
                   "the recorded event is the canonical :structure kind")
      (check-equal '("acme/work/n") (node-deps (kernel-state k) "acme/work/d")
                   "and does not double the edge")
      (check-equal '("acme/work/d") (node-dependents (kernel-state k) "acme/work/n")
                   "nor the reverse edge")
      ;; and the no-effect event replays to the same tree
      (let ((fresh (reconstruct-state
                    (canonical-string (state-canonical-form (kernel-state k))))))
        (check-equal '("acme/work/n") (node-deps fresh "acme/work/d")
                     "a canonical replay of the no-effect event moves no edge"))
