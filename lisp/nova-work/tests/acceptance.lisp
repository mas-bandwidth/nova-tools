...
;;; ------------------------------------------------------------------
;;; W1: the resume predicate (bench identity + journal lock),
;;;     the single writer, and the handoff successor.
;;; ------------------------------------------------------------------

(defun journal-seq-numbers (path)
  "Read the frame sequence numbers written to journal file PATH, in order."
  (let ((seqs '()))
...
(deftest "ownership-record-round-trips" "docs/SPEC-WORK.md:154-157"
    "expected=parse(format(x))=x,bench-included"
  (let ((rec (make-ownership-record
              ...
              :successor "rowan")))
    (let ((parsed (parse-ownership-record (format-ownership-record rec))))
...
(deftest "handoff-successor-takes-next-generation" "docs/SPEC-WORK.md:205-208,814-827"
    "expected=successor-takes-now;generation-bumped;fresh-token;successor-bench-recorded"
  (let ((current (make-ownership-record
                  :owner "emma"
                  :generation 4
                  :token "tok-emma-old"
                  :stamp "2026-09-14T11:59:00Z"
                  :until "2026-09-14T11:00:00Z"
                  :bench "bench-old"
                  :successor "rowan")))
    (multiple-value-bind (action record line)
        (evaluate-ownership-claim current "rowan"
                                  :now "2026-09-14T12:00:00Z"
                                  :every "30s"
                                  :skew "5s"
                                  :token "tok-rowan-new"
                                  :my-bench "bench-rowan")
      (declare (ignore line))
      (check-equal :take action "a named successor takes at once")
      (check-equal 5 (owner-generation record) "generation bumps to the next one")
      (check-string= "rowan" (owner-owner record) "the successor is now the owner")
      (check-string= "tok-rowan-new" (owner-token record) "the successor draws a fresh token")
      (check-string= "bench-rowan" (owner-bench record) "the successor's bench is recorded"))))
...
(deftest "resume-refuses-a-copied-journal" "docs/SPEC-WORK.md:193-204"
  ...
  (let ((current (make-ownership-record
                  ...
                  :until "2026-09-14T12:01:00Z"
                  :bench "bench-a")))
    ;; Case 1: same owner and token, but a different bench wrote the journal
    ;; (a copied journal grants nothing on another bench).
    (multiple-value-bind (action record line exit-code)
        (evaluate-ownership-claim current "emma"
                                  :now "2026-09-14T12:00:30Z"
                                  :every "30s"
    ...
    ;; Case 2: same owner, token and bench, but the journal's lock is not held.
    (multiple-value-bind (action record line exit-code)
        (evaluate-ownership-claim current "emma"
                                  :now "2026-09-14T12:00:30Z"
                                  :every "30s"
...
(deftest "resume-needs-the-journal-lock" "docs/SPEC-WORK.md:162-183,193-204"
  ...
  (let* ((path (test-journal-path "resume-lock"))
    ...
    (unwind-protect
         (progn
           ;; A second flock on the held <journal>.lock must fail.
           (ok (null (take-journal-lock path))
               "a second flock on the held lock must fail")
           ;; Opening the journal refuses while another process holds its lock.
           (let ((held nil))
             (handler-case (open-file-journal path)
               (journal-held () (setf held t)))
             (ok held "open-file-journal did not refuse the held lock"))
           ;; The resume predicate refuses when the lock is not this holder's.
           (let ((current (make-ownership-record
  ...
  (let* ((path (test-journal-path "serialize"))
    ...
    (unwind-protect
         (progn
           ...
           ;; Total order: every command recorded exactly once.
           ...
           ;; Consecutive sequence numbers, in one order.
           (check-equal (loop for n from 1 to 20 collect n)
                        (journal-seq-numbers path)
