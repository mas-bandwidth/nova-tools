...
(defun read-verification-cache (path &key resolvers max-bytes max-depth max-nodes session)
...
installs nothing: the cache is returned only when every line passed."
  (let ((cache (make-verification-cache :resolvers resolvers)))
    ...
    (let ((text (read-bounded-file path :max-bytes max-bytes :max-depth max-depth :max-nodes max-nodes
                                        :session session
