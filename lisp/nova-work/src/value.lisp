...
(defun roadmap-bug-counts (form)
  "Extract bug counts from a roadmap s-expression FORM. Returns a plist with
:open, :fixed, :features and :items. Walks :bugs lists at the root, epic,
feature and item levels; :current-bugs at the root is the authority."
  (labels ((%bug-open-fixed (node)
             (let ((open 0) (fixed 0))
               (dolist (bug (getf node ':bugs))
                 (let ((status (getf bug ':status)))
                   (when (eq ':open status) (incf open))
                   (when (eq ':fixed status) (incf fixed))))
               (values open fixed)))
           (%walk-bugs (node)
             (multiple-value-bind (o f) (%bug-open-fixed node)
               (let ((total-o o) (total-f f))
                 (dolist (epic (getf node ':epics))
                   (multiple-value-bind (eo ef) (%walk-bugs epic)
                     (incf total-o eo)
                     (incf total-f ef)
                     (dolist (feature (getf epic ':features))
                       (multiple-value-bind (fo ff) (%walk-bugs feature)
                         (incf total-o fo)
                         (incf total-f ff)
                         (dolist (item (getf feature ':items))
                           (when (listp item)
                             (multiple-value-bind (io i-f) (%walk-bugs item)
                               (incf total-o io)
                               (incf total-f i-f))))))))
               (values total-o total-f)))))
    (multiple-value-bind (walked-open walked-fixed) (%walk-bugs form)
      (let ((tail (member ':current-bugs form)))
        (list :open (if (and tail (cadr tail))
                        (let ((v (cadr tail)))
                          (if (listp v) (first v) v))
                        walked-open)
              :fixed (if (and tail (cadr tail))
                         (let ((v (cadr tail)))
                           (if (listp v)
                               (or (second v) 0)
                               (or (and (cddr tail) (first (cddr tail))) 0)))
                         walked-fixed)
              :features (getf form ':current-features)
              :items (getf form ':current-acceptance-items))))))
