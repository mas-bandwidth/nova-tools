...
;;; ------------------------------------------------------------------
;;; E10-F04-01 — orchestrate process-level runs against a temporary
;;; remote and a fake provider (SPEC-WORK.md:7223-7227, ROADMAP.md:1014)
;;; ------------------------------------------------------------------
;;;
;;; "process-level fault injection and restart-and-replay against temporary
;;; Git remotes and fake providers, two-process fencing and interrupted I/O
;;; included." The four process-level moves — crash/restart, partition,
;;; stale owner and handoff — are driven against one temporary clip-remote
;;; (the in-process Git branch) and one fake decider (the in-process provider),
;;; and the single-writer invariant is read back from the remote, the provider
;;; and the ownership claims rather than remembered. Nothing here opens a
;;; socket or touches a repository.

(defun orchestrate-process-level-run (&key (tip "genesis")
                                           (t0 "2026-09-20T12:00:00Z")
                                           (t1 "2026-09-20T12:00:30Z"))
  "Orchestrate one process-level lifecycle across the four failures the staged
verification names. Answers a report plist whose values are the outcomes read
back from the temporary remote, the fake provider and the ownership claims."
  (let* ((remote (make-clip-remote :tip tip))
    ...
    (multiple-value-bind (vacant-action vacant-record)
      ...
      ;; 2. Stale owner: a non-successor contests the live hold and is fenced.
      (multiple-value-bind (stale-action)
          (evaluate-ownership-claim
           ...
           :my-bench "bench-zoe")
        ;; 3. Handoff: the record names stella successor; stella takes gen 2 now.
        (multiple-value-bind (handoff-action handoff-record)
          ...
          ;; 4. Partition (two-process fencing): stella's push lands and moves
          ;;    the tip; the stale emma, still believing the old tip, is refused
          ;;    by the compare-and-swap.
          (let ((stella-pushed (clip-remote-push remote tip "commit-stella"))
                (stale-pushed (clip-remote-push remote tip "commit-emma")))
            ;; 5. The fake provider answers the bounded question exactly once.
            (multiple-value-bind (answer confidence)
                (decide-consult provider '(:kind :choice :options (:run :stop)))
              (declare (ignore confidence))
              ;; 6. Crash/restart and once-only: a fresh kernel over the same
              ;;    journal answers the recorded receipt and applies nothing a
              ;;    second time.
              (let* ((journal (make-ordering-journal))
                     (live (make-kernel :state (make-seed-state seed)
                                        :journal journal))
                     (request (list :verb :state-to-doing :node "w/t1" :by "emma"
                ...
                (multiple-value-bind (okp line)
                    (submit live request)
                  (declare (ignore line))
                  (let* ((restarted (make-kernel :state (make-seed-state seed)
                                                 :journal journal))
                         (retry-envelope (nth-value 3 (submit restarted request))))
                    (list :vacant-action vacant-action
                          ...
                          :stella-pushed stella-pushed
                          :stale-pushed stale-pushed
                          :remote-tip (clip-remote-tip remote)
                          :remote-pushes (clip-remote-pushes remote)
                          :provider-answer answer
                          :provider-calls (fake-decider-calls provider)
                          :restart-live-ok okp
                          :restart-live-state (node-state (kernel-state live) "w/t1")
                          :restart-replayed (getf retry-envelope :replayed)
