;;;; sha256.lisp --- SHA-256, in the system itself.
;;;;
;;;; docs/SPEC-WORK.md:325 names the digest: "it is SHA-256, printed as
;;;; lowercase hex, over the canonical serialization of the request's payload".
;;;; It is written here rather than taken from a library so the kernel has no
;;;; dependency outside the implementation, and it is checked against the
;;;; published vectors in the suite.
  ...
  (let* ((len (length bytes))
    ...
    (loop for base from 0 below total by 64 do
      ...
      (loop for i from 16 below 64 do
        (let* ((a (aref w (- i 15)))
               (b (aref w (- i 2)))
               (s0 (logxor (rotr32 a 7) (rotr32 a 18) (ash a -3)))
               (s1 (logxor (rotr32 b 17) (rotr32 b 19) (ash b -10))))
      ...
      (let ((a (aref h 0)) (b (aref h 1)) (c (aref h 2)) (d (aref h 3))
        ...
        (dotimes (i 64)
          (let* ((s1 (logxor (rotr32 e 6) (rotr32 e 11) (rotr32 e 25)))
                 (ch (logxor (logand e f) (logand (u32 (lognot e)) g)))
...
(defun sha256-hex (input)
  "Lowercase hex SHA-256 of INPUT, a string (UTF-8) or an octet vector."
  (let ((h (sha256-octets (if (stringp input) (utf8-octets input) input))))
    (string-downcase (with-output-to-string (s)
                       (loop for word across h do (format s "~8,'0X" word))))))
