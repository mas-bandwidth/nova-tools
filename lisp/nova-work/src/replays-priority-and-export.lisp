...
(defun priority-slot (table id context)
...
(defun (setf priority-slot) (value table id context)
...
containment path, else the default (SPEC-WORK.md:3172)."
  (let ((self (priority-slot table id :self)))
  ...
  (let ((cursor (priority-parent table id)))
    (loop while cursor do
      (let ((sub (priority-slot table cursor :subtree)))
...
(defun priority-set (table id context rank reason change-id)
...
no-effect receipt (SPEC-WORK.md:3187)."
  (unless (rank-atom-p rank)
    (return-from priority-set (values nil (format nil "bad rank ~S" rank) 2)))
  (let ((current (priority-slot table id context)))
  ...
  (let ((current (priority-slot table id context)))
    (if (eq current +absent+)
        (values (make-priority-receipt :change :clear :context context :rank +absent+
                ...
                "PRIORITY OK changed=0" 0)
        (progn
          (setf (priority-slot table id context) +absent+)
          (note-priority-change table id context change-id)
          (values (make-priority-receipt :change :clear :context context :rank +absent+
                                         :changed 1 :no-effect nil :change-id change-id)
                  "PRIORITY OK changed=1" 0)))))
...
(defstruct (operation (:constructor %make-operation (&key id kind revision status result))
                      (:conc-name op-))
  id kind revision status result)

(defstruct (operation-registry (:constructor make-operation-registry (&key (counter 0))))
  counter
  ...
  (when inside-batch
    (return-from begin-operation
      (values nil (format nil "OPERATION FAIL: ~A refused inside atomic batch ~A"
                          kind entry-id) 2)))
  (let ((id (format nil "op-~D" (incf (operation-registry-counter registry)))))
    (setf (gethash id (operation-registry-operations registry))
          (%make-operation :id id :kind kind :revision revision :status :queued))
    (values id (format nil "OPERATION OK id=~A op=~A state=queued" id kind) 0)))

(defun operation (registry id)
  (gethash id (operation-registry-operations registry)))

(defun operation-state (registry id)
  (let ((op (operation registry id)))
    (and op (op-status op))))

(defun operation-start (registry id)
  (let ((op (operation registry id)))
    (when op (setf (op-status op) :running))
    op))

(defun operation-finish (registry id &key result)
  (let ((op (operation registry id)))
    (when op (setf (op-status op) :done (op-result op) result))
    op))

(defun operation-cancel (registry id)
...
(defun operation-wait (registry id)
