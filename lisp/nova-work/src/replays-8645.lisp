...
(defun goal-show (world &key scope)
...
(defun goal-update (world &key scope expect (form :progress) reason
...
`state --to doing` `stop requested`."
  (when (null expect)
    (return-from goal-update (values nil "GOAL FAIL: --expect is required" 2)))
  ...
  (let ((goal (gw-goal world scope)))
    (unless goal
      (return-from goal-update (values nil "GOAL FAIL: no goal in scope" 1)))
    (let ((state (gw-state world goal)))
      (ecase form
        (:stop
         (unless (member state *stop-edges*)
           (return-from goal-update (values nil "GOAL FAIL: no edge" 1)))
...
(defun goal-set (world &key scope goal expect request reason clear)
...
The disposition refusal is unconditional; a retried request replays its event."
  ...
  (when (and (not clear) goal)
    (let ((state (gw-state world goal)))
      (cond ((null state)
             (return-from goal-set (values nil "GOAL FAIL: no such node" 1)))
            ((eq state :done)
             (return-from goal-set (values nil "GOAL FAIL: disposition=done" 1)))
            ((eq state :cancelled)
             (return-from goal-set (values nil "GOAL FAIL: disposition=cancelled" 1))))))
