...
revived to be counted once either way \"with no rule 18 finding on any of
them\". Rows are newest first, so the first row naming an id is its latest: a
`:revive` there is the kernel's own record that the id honestly left C, and it
is no finding."
