...
(defstruct (fleet (:constructor make-fleet (&key machine (allocations nil))))
  machine allocations)

(defun fleet-occupied-slots (fleet)
  (loop for a in (fleet-allocations fleet)
...
(defun fleet-active-slots (fleet)
  (reduce #'+ (remove-if-not #'allocation-active-p (fleet-allocations fleet))
          :key #'allocation-slots :initial-value 0))

(defun fleet-take (fleet &key node slots machine-generation holder allocation-id)
...
capacity line at exit 1 (SPEC-WORK.md:6150)."
  (let* ((machine (fleet-machine fleet))
         (requested (or slots 1))
         (used (fleet-active-slots fleet)))
    (cond
      ((and machine-generation
            (/= machine-generation (machine-generation machine)))
       (values nil (format nil "ALLOC FAIL machine=~A slots=~D holder=~A: capacity"
                           (machine-id machine) requested holder)
               1 nil))
      ((> (+ used requested) (machine-concurrent machine))
       (values nil (format nil "ALLOC FAIL machine=~A slots=~D holder=~A: capacity"
                           (machine-id machine) requested holder)
               1 nil))
      (t
       (let* ((slot (fleet-first-free-slot fleet))
              (alloc (make-allocation :id allocation-id :machine (machine-id machine)
                                      :slot slot :slots requested :node node
                                      :generation (machine-generation machine)))
              (new (make-fleet :machine machine
                               :allocations (append (fleet-allocations fleet)
                                                    (list alloc)))))
         (values t
                 (format nil "ALLOC OK allocation=~A machine=~A slot=~D node=~A"
                         allocation-id (machine-id machine) slot node)
                 0 new))))))

(defun fleet-release (fleet &key allocation generation)
...
(SPEC-WORK.md:6119-6123, 6147-6149)."
  (let ((a (find allocation (fleet-allocations fleet)
    ...
    (cond
      ((or (null a) (not (allocation-active-p a)))
       (values nil (format nil "ALLOC FAIL machine=~A allocation=~A: stale token"
                           (machine-id (fleet-machine fleet)) allocation)
               1 nil))
       ...
       (let ((released (copy-allocation a)))
         ...
         (values t
                 (format nil "ALLOC RELEASE OK allocation=~A machine=~A slot=~D freed=true"
                         (allocation-id a) (machine-id (fleet-machine fleet))
                         (allocation-slot a))
                 0
                 (make-fleet :machine (fleet-machine fleet)
                             :allocations (substitute released a
                                                      (fleet-allocations fleet)))))))))

(defun fleet-heartbeat (fleet &key allocation generation)
    ...
    (if (and a (allocation-active-p a)
        ...
        (values t
                (format nil "ALLOC HEARTBEAT OK allocation=~A machine=~A"
                        allocation (machine-id (fleet-machine fleet)))
                0)
        (values nil (format nil "ALLOC FAIL machine=~A allocation=~A: stale token"
                            (machine-id (fleet-machine fleet)) allocation)
                1))))

(defun fleet-list (fleet)
