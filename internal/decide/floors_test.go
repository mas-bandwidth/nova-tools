...
func TestTheOneFloorWasAboveEveryProviderAnswer(t *testing.T) {
	...
	// escalation rate and the end of "the lowest rung the evidence supports".
	uniform := &Registry{Minds: reg.Minds, Floors: uniformFloors(Kinds, 0.90)}
	sum, err := Summarize(uniform, dayLog(t))
	if err != nil {
		t.Fatal(err)
	}
...
// uniformFloors is one floor standing in for every kind, the defect the per-kind
// table replaces.
func uniformFloors(kinds []string, floor float64) []KindFloor {
	out := make([]KindFloor, 0, len(kinds))
	for _, kind := range kinds {
		out = append(out, KindFloor{Kind: kind, Floor: floor})
	}
	return out
}
