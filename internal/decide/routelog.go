...
// outcome rather than a policy. The latency tokens are appended last on the
// lines they belong to, so every field before them keeps its name, position and
// spelling: per-kind median and p95 on the kind's own line, per-decider median
// and p95 on the finish.
func (s Summary) Render() string {
	...
	for _, k := range s.Kinds {
		...
		l := latByKind[k.Kind]
		fmt.Fprintf(&b, "LOG kind=%s decisions=%d escalations=%d successes=%d failures=%d start_rung=%s start_height=%d default_rung=%s regenerated=%v floor=%.2f floor_from=%s provider_rows=%d conf_min=%s conf_max=%s conf_p25=%s below_floor=%d defeated=%v hist=%s lat_n=%d median_ms=%s p95_ms=%s\n",
	...
	fmt.Fprintf(&b, "LOG OK rows=%d kinds=%d escalations=%d defeated=%d coverage=%d/%d",
		s.Entries, len(s.Kinds), escalations, defeated, s.Outcomes, s.Decisions)
