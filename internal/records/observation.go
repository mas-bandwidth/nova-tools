...
func (v *Validator) receipt(body *Object) (map[string]string, error) {
	...
	for _, name := range o.Keys() {
		...
		if !allow[name] {
			...
			if err := v.refused(RuleReceiptNotAllowed, fp, "a receipt carries only mapping-allowlisted native locator fields"); err != nil {
				return nil, err
			}
		}
		val, _ := o.Get(name)
		str, ok := val.(string)
		if !ok {
			return nil, refuse(RuleWrongType, fp, "a receipt value is a string")
		}
		out[name] = str
	}
	return out, nil
