...
func RunSeal(opts SealOptions) (string, error) {
	...
	existing, err := sealDecrypt(opts.SopsPath, opts.KeyPath, targetFile)
	if err != nil {
		return "", err
	}
	plaintext := sealApply(existing, opts.Name, value)

	ciphertext, err := sealEncrypt(opts.SopsPath, opts.KeyPath, seatFile, plaintext)
	if err != nil {
		return "", err
	}
	...
	if err := sealGit(opts.StoreDir, opts.GitPath, "checkout", "-b", branch); err != nil {
		return "", err
	}
	if err := sealGit(opts.StoreDir, opts.GitPath, "add", seatFile); err != nil {
		return "", err
	}
	commitMsg := fmt.Sprintf("seal %s into %s", opts.Name, seatFile)
	if err := sealGit(opts.StoreDir, opts.GitPath, "commit", "-m", commitMsg); err != nil {
		return "", err
	}
	...
	if err := sealGit(opts.StoreDir, opts.GitPath, "push", "-u", "origin", branch); err != nil {
		return "", err
	}
	...
	createOut, err := sealGH(opts.GHPath, opts.StoreDir, "pr", "create", "--head", branch, "--title", title, "--body", body)
	if err != nil {
		return "", err
	}
	prNum := ""
	...
	for {
		view, err := sealGH(opts.GHPath, opts.StoreDir, "pr", "view", prNum, "--json", "reviewDecision", "--jq", ".reviewDecision")
		if err != nil {
			return "", err
		}
	...
	if _, err := sealGH(opts.GHPath, opts.StoreDir, "pr", "merge", prNum, "--squash"); err != nil {
		return "", err
	}
	if err := sealGit(opts.StoreDir, opts.GitPath, "pull"); err != nil {
		return "", err
	}
...
func sealDecrypt(sopsPath, keyPath, filePath string) ([]byte, error) {
	...
	cmd.Env = sealSopsEnv(keyPath, tmpDir)
...
func sealEncrypt(sopsPath, keyPath, seatFile string, plaintext []byte) ([]byte, error) {
	tmpDir, err := os.MkdirTemp("", "nova-secrets-seal-*")
	if err != nil {
		return nil, fmt.Errorf("failed to create temporary isolation directory: %w", err)
	}
	...
	cmd := exec.Command(sopsPath, "-e", "--filename-override", seatFile, "--input-type", "yaml", "--output-type", "yaml")
	cmd.Env = sealSopsEnv(keyPath, tmpDir)
...
func sealGit(dir, gitPath string, args ...string) error {
...
func sealGH(ghPath, dir string, args ...string) (string, error) {
