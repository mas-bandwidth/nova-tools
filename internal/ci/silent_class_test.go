	...
	for key := range allow {
		if !seen[key] {
			violations = append(violations, fmt.Sprintf(
				"%s lists %s, but no `_ = err` or `|| true` literal is there any more; delete the stale entry (the list only shrinks)",
				silentAllowlistPath, key))
		}
	}
...
func readSilentAllowlist(t *testing.T) map[string]bool {
	t.Helper()
	raw, err := os.ReadFile(silentAllowlistPath)
	if err != nil {
		t.Fatal(err)
	}
