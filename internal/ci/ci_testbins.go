...
func varKey(id *ast.Ident) any {
	if id.Obj != nil {
		return id.Obj
	}
	return "unresolved:" + id.Name
}

// readByteVars collects the variables a file fills from os.ReadFile, so a
...
// carries the taint to its end. Taint is bound to the variable (varKey), never
// to the identifier's spelling.
func readByteVars(file *ast.File) map[any]bool {
...
// execFileVars collects the variables a file opens with an execute bit
// through os.Create or os.OpenFile, so an io.Copy into one is a copy to an
// executable destination. Like readByteVars it keys by variable, not spelling.
func execFileVars(file *ast.File) map[any]bool {
	vars := map[any]bool{}
	ast.Inspect(file, func(n ast.Node) bool {
...
func taintedExpr(e ast.Expr, tainted map[any]bool) bool {
	switch x := e.(type) {
	case *ast.ParenExpr:
...
func taintLHS(lhs ast.Expr, tainted map[any]bool) bool {
	switch x := lhs.(type) {
	case *ast.Ident:
		k := varKey(x)
		if tainted[k] {
...
// execute bit.
func isExecFile(e ast.Expr, execFiles map[any]bool) bool {
	switch x := e.(type) {
	case *ast.ParenExpr:
		return isExecFile(x.X, execFiles)
	case *ast.Ident:
		return execFiles[varKey(x)]
	}
	return false
}
