...
var admittingEventKinds = map[string]bool{
	...
	// literally. The first cut listed only the kinds the SHIPPED file happens
	// to use, so a schema carrying an unmarked verb of kind :lease, :handoff,
	// :reassign, :allocation or :packet passed the coverage test. The closed
	// list is the closed list.
	":lease":       true,
	":handoff":     true,
	":reassign":    true,
	":offer":       true,
...
//
// A missing verb with an admitting kind is exactly the "later verb" rule 3
// wants caught (SPEC-WORK.md:4878); that it is not built yet is what its
// `status` says, not a reason to skip it.
...
// TestShippedSchemaSurvivesAnInjectedUnmarkedVerb runs the shipped file plus
...
// TestAMissingVerbWithAnAdmittingKindIsCaught flips an EXISTING `missing_verbs`
