...
// internal/ci's TestEverySubVerbFlagSetAnswersHelp states in source: `nova-update
...
//
// Two binaries are built on this package and they print different verbs, so
// each is asked for its own: nova-update's five plus the four release verbs
// internal/release owns, and nova-version's four.
func TestEverySubVerbAnswersHelp(t *testing.T) {
	for tool, verbs := range map[string][]string{
		"nova-update":  {"check", "apply", "report", "watch", "adoption", "release cut", "release build", "release install", "release adopt"},
		"nova-version": {"snapshot", "diff", "report", "send"},
	} {
		for _, verb := range verbs {
			for _, spelling := range []string{"--help", "-h"} {
				args := append(strings.Fields(verb), spelling)
				var out, errs bytes.Buffer
				code := Run(tool, args, "test", &out, &errs, Environment{})
				assertSubVerbHelp(t, tool, verb, spelling, code, out.String(), errs.String())
			}
		}
	}
}
