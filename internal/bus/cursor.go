...
func ValidOpenID(id string) error {
	...
	if validLegacyOpenID(id) {
		return nil
	}
	return ValidID(id)
}

func validLegacyOpenID(id string) bool {
	if len(id) == 0 || len(id) > SlugMax || id == "-" {
		return false
	}
	for i := 0; i < len(id); i++ {
		c := id[i]
	...
	return !isLegacyPunct(id[0]) && !isLegacyPunct(id[len(id)-1])
}

func isLegacyPunct(c byte) bool {
	return c == '-' || c == '_' || c == '.'
}
