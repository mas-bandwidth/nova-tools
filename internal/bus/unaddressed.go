...
// it is on the table, and no reader is ever shown it -- with the quietest cause of the lot.
...
// precisely the writing the table's form is meant to allow.
//
// So they are reported:
//
//   - on `--full`, to EVERY reader, because a full read is the one that says what is on the
//     table rather than what is new for me, and one of these is on the table and in nobody's
//     inbox;
...
// Unaddressed is one note on the table that resolves to no recipient at all.
...
func UnaddressedReason(c *Config, n *Note) (string, bool) {
	...
	return fmt.Sprintf("%s: %s names no one at this table, so this note is in nobody's inbox (known: %s)",
		KeyTo, quoteAll(unknown), strings.Join(c.KnownNames(), "; ")), true
}

// UnaddressedOnTable is every note on the table that reaches no reader, which is what a
// full read reports. It is the same list for everybody, deliberately: the fact is about the
// note and not about who is looking.
func (t *Table) UnaddressedOnTable() []Unaddressed {
	var out []Unaddressed
	for i := range t.Notes {
		n := &t.Notes[i]
