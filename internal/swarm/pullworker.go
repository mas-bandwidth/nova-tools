...
func RunPullWorker(ctx context.Context, opts PullWorkerOptions) int {
	...
	for {
		...
		res, err := PullLease(opts.Store, leaseOwner, opts.For, now, os.Getpid())
		...
		// The label names the workspace directory below the store and the harvest
		// path below that. A queue/ is the entry point for untrusted names, so a
		// label that is not one safe path element must not be joined anywhere.
		if !safepath.NameOK(label) {
			takenPath := filepath.Join(opts.Store, TakenName, leaseOwner+"-"+res.Card)
			fmt.Fprintf(opts.Stderr, "nova-swarm pull: refusing card=%s: label is not a safe name\n", oneline.Field(res.Card))
		...
		// Terminal outcome storage BEFORE releasing lease.
...
func harvestResult(workDir, harvestPath string) {
	...
	for _, cand := range candidates {
		data, err := readResultFile(workDir, cand)
...
// readResultFile reads cand only when it sits strictly below workDir and is not
// itself a symlink. The card runs in a container that writes into workDir, and a
// link planted there would carry this host-side read out of the job, so the read
// is refused rather than followed.
func readResultFile(workDir, cand string) ([]byte, error) {
	resolved, err := safepath.ResolvedUnder(cand, workDir)
	if err != nil {
		return nil, err
	}
