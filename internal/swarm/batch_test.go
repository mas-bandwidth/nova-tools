...
func TestIdleWatchCountsChildActivity(t *testing.T) {
	...
	// red run of this test leaves no process behind. It is a GRANDCHILD of the card, which
	// is the point -- the monitor reads the whole process tree's CPU time.
	// The marker appears once the spin card's grandchild has burned, so the test waits on
	// that event rather than on a clock. Before it takes its first activity sample it waits
	// for that grandchild to appear in the tree with CPU accrued -- a slow fork on a loaded
	// runner must not be read as idle -- and the sample taken after the marker then sees the
	// burner gone: the tree's CPU time did not grow, it fell, and a tree that lost a charged
	// process is working, not still. The burn only has to exceed a hundredth of the INJECTED
	// sample interval -- the idle window the test advances is four virtual seconds, so forty
	// milliseconds of CPU is already activity -- and is a fraction of a second, never the
	// five real seconds it used to hold a core for. The sleeping card has no such movement,
	...
	runner := runnerDoing(t, dir, "silent",
		...
		runnerStep{Op: "spin", N: 20000, Ms: 300},
