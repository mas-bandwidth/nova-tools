...
func Batch(in BatchInput) int {
	...
	// The slot lock, taken at allocation and removed at slot end: one batch holds one slot
	// (issue #457). A live lock refuses that slot for the card that named it -- that card
	// alone -- and a stale lock is taken over once, out loud.
	var owned []string
	defer func() {
	...
	for i := range cards {
		...
		why, note, err := takeSlotLock(in.Root, cards[i], in.ID)
		if err != nil {
			fmt.Fprintf(in.Stderr, "nova-swarm batch: %s\n", oneline.Err(err))
			return 2
		}
		...
		if why != "" {
			cards[i].admitWhy = why
			fmt.Fprintln(in.Stderr, "ADMIT REFUSED "+why)
			continue
		}
		owned = append(owned, slotLockPath(in.Root, cards[i]))
	}
	...
	for i, c := range cards {
		...
		state, reason, tail, line2 := scoreCard(in.Root, c, procs[i].idleKilled, procs[i].deadKilled, procs[i].rc, idleSeconds, logPath)
...
func scoreCard(root string, c batchCard, idleKilled, deadKilled bool, rc, idleSeconds int, logPath string) (state, reason, tail, line2 string) {
	switch {
	case idleKilled:
		return "abstain", fmt.Sprintf("idle=%d", idleSeconds), "watched=" + logPath, ""
	case deadKilled:
		return "abstain", "deadline", "", ""
	case cardEndsInputLimit(logPath):
		return "abstain", "input-limit", "", ""
	}
	job := slotJobDir(root, c.slot, c.label)
...
func slotLocked(root string, n int) bool {
	if _, live := slotHeldBy(root, n); live {
		return true
	}
