...
// Bench is one row of the benches table. auth is parsed and ignored; the other six columns
// are carried, but only the fields the slices read are populated here.
type Bench struct {
	...
	Wall    string // "sandbox" or "none": the wall the bench may run
}

// remoteKillDelay is how long the wrapper waits between the remote SIGTERM and the SIGKILL:
// ssh <host> pkill -TERM -g <pgid>, then -KILL after this long.
const remoteKillDelay = 5 * time.Second
...
// native ..., with the ssh child in a process group of its own. A wall=none bench whose batch
// typed --no-wall passes that flag through to native.
func remoteRun(c batchCard, b Bench, localRoot string, deadline int, noWall bool, logFile *os.File) (*exec.Cmd, error) {
...
// remoteLogPath is a remote card's own log, the file the idle watch measures growth by:
// <root>/<n>/jobs/<label>/native.log, the bench's own slot number, never the local scratch.
...
// pkillRemote asks the bench to signal one remote process group: ssh <host> pkill -<sig> -g <pgid>.
func pkillRemote(host, sig, pgid string) error {
	cmd := exec.Command("ssh", host, "pkill", "-"+sig, "-g", pgid)
	return cmd.Run()
}

// statRemote reads one remote file's byte size: ssh <host> stat -c %s <path>. A failed stat is
// an unreachable bench, never a zero-length log.
func statRemote(host, path string) (int64, error) {
...
// remotePgid extracts the process group the remote native runs under from its first line,
// "RUN pgid=<n>", printed before any card work so a batch can name it for a kill.
...
// killRemote ends a remote card at the deadline or on idle: SIGTERM to the ssh child's own
// process group first, then ssh <host> pkill -TERM -g <pgid> with the pgid native reported on
// its RUN pgid= line, then -KILL after remoteKillDelay.
func killRemote(pid int, host, pgid string) {
	TerminateGroup(pid, "")
	if host != "" && pgid != "" {
		_ = pkillRemote(host, "TERM", pgid)
