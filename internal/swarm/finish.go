...
func (in RunInput) finish(r *running, retired map[int]bool, now time.Time) (string, string, string) {
	...
	// BUT exit.json IS an answer: when the supervisor's own completion evidence carries
	// this launch's nonce and attestation, the job's end is confirmed and the slot is free
	// (run 34698330796, `TestANumericBudgetWithNoUsageSourceIsRefused`, the Windows flake
	// where a slot collision retired a green job and made the run exit 1 over nothing).
