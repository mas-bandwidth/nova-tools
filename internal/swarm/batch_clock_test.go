...
// its second tick).
//
// Both waits are also bounded by testWait (nova-tools #1983): a tick nothing is left
// to receive is dropped, not held, so one stuck test cannot hang the whole package.
