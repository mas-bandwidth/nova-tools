...
func TestBatchCardsHandsTheRunnerTheWordAsItsSixthArgument(t *testing.T) {
	...
	// The deadline is the manual clock's, which nothing advances: the card ends
	// when its runner does, never because a loaded machine took ten real seconds
	// to start it (the branch runs this failed on, nova-tools#4328).
	clk := newManualClock()
	code, _, errb := runBatchClock(BatchInput{
		ID: "TB4", Deadline: 10 * time.Second, Cards: tsv, Root: root,
		Runner: runner, Tokens: "90000",
	}, clk, func() {})
	if code == 2 {
		t.Fatalf("the batch refused admission: %s", errb)
	}
