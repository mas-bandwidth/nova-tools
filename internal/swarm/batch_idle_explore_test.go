...
// signal, and a harness waiting on an HTTP response spends none of it -- the
// 1%-of-interval threshold that separates a working tree from bookkeeping jitter is exactly
// what a blocked-on-the-network process does not clear.
//
...
func TestIdleWatchCountsWALCommitAtConstantSize(t *testing.T) {
	...
	runner := runnerDoing(t, dir, "wal",
		...
		// AND THE PUBLISH IS A SECOND GRACE BEHIND THAT, one the reap is designed to
		// outlive (the linux cold read of this recut, 2026-09-21). The monitor's kill
		// fires inside the tick; the reap's TERM reaches this tree a moment later -- on
		// linux, a whole /proc scan later, which a publish that lands within a few
		// milliseconds of the tick can beat. A card that had already published by the
		// time gather reads it scores DONE whatever the monitor decided, and the defect
		// hides: the first cut of this test passed RED on a linux bench that way. So the
		// card keeps working a while after its last commit before it publishes -- which
		// is also just the shape of a real harness -- and a TERM sent anywhere inside
		// that window ends the card before the file exists. The window is a fraction of
		// the house's own reap bound (TerminateGrace), never a bound the test asserts.
		runnerStep{Op: "sleep", Ms: 1000, When: "label==committing"},
		publishCard("{job}"),
	)
