...
func WallDeath(jobDir, task string) (string, bool) {
	...
	return WallDeathFrom(raw, jobDir, task)
}
...
// WallDeathFrom is WallDeath asked of bytes the caller already holds -- the parent
// tee -- rather than a re-read of a path inside --write (issue #1892).
func WallDeathFrom(raw []byte, jobDir, task string) (string, bool) {
...
// (issue #1465, Stella's HOLD on PR #1478, and issue #1892).
//
