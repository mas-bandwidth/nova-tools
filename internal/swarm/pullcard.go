...
// label. A worker renews that lease while the card runs.
//
...
// still alive is DRIFT -- never reaped and never regranted.
...
func QueuedCards(store string) ([]string, error) {
	entries, err := os.ReadDir(filepath.Join(store, QueueName))
	if err != nil {
	...
	for _, e := range entries {
		if e.IsDir() || !strings.HasSuffix(e.Name(), CardExt) {
			continue
		}
...
// PullLease takes the next card from <store>/queue under a fresh one-slot lease.
...
func PullLease(store, owner string, dur time.Duration, now time.Time, pid int) (PullResult, error) {
	...
	_ = granted
	takenDir := filepath.Join(store, TakenName)
	if err := os.MkdirAll(takenDir, 0o755); err != nil {
		_, _, _ = ReleaseSlotLeases(store, owner, card, false)
		return PullResult{}, err
	}
	if err := os.Rename(filepath.Join(store, QueueName, card), filepath.Join(takenDir, owner+"-"+card)); err != nil {
		_, _, _ = ReleaseSlotLeases(store, owner, card, false)
		return PullResult{}, err
	}
