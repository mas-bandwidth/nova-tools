...
// every verb but quickstart and add, which make it (MakeDir below). For the rest: a
...
// clone, is the caller's decision and not this tool's. SPEC-BOARD grants the directory
// backend one creation and names it: "One file per card, <dir>/<id>.board, created by
// add." The FIRST add is the other side of that sentence: an empty directory is a valid
// empty ledger, so the add that files the first card makes the directory it writes into
// (issue 625).
//
...
// MakeDir is NewDir for QUICKSTART AND THE FIRST ADD, and it is the only entry here that
// creates the directory. Glenn ruled it on nova-tools #109, on Emma's report that
// quickstart refused a --dir that was not there: "It is best to do the right thing if a
// friend uses it a certain way, or to correct docs to show only right way. Pick one."
// The right thing is the one a friend already reached for -- `nova-swarm quickstart
// --pool ./pool` makes its pool, with MkdirAll and 0755, and this makes the board
// directory the same way. Issue 625 extends the ruling to the FIRST add: the board's
// first card has nowhere to write yet, an empty directory is a valid empty ledger, and
// `add --dir <new board> --as ...` is the dogfooded first use (the issue's `--what`
// spelling is not evidence; the flag is `--text`).
//
// IT IS QUICKSTART'S AND THE FIRST ADD'S ALONE. Every other verb goes through NewDir and
// still refuses, because quickstart is the verb whose whole job is a first run and add is
// the verb that writes the first card -- a first run has nowhere to write yet, while a
// `list` or a `take` against a directory that is not there is a caller who named
// the wrong path -- making it for them would answer a typo with an empty board. The
// created bool goes back to the caller because a verb that makes a directory silently
// leaves a reader unable to tell a new board from the wrong one.
//
