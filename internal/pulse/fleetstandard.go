...
func FleetStandardChecks(goos, goWant, stamp string, minFreeGB int) []StandardCheck {
	...
	all := []StandardCheck{
		...
		// A darwin bench is not provisioned until Spotlight is off: #1432 measured the same
		// package on the same commit at 68.1/72.7/73.8/74.2/81.2 s because indexing runs
		// over the clone, the build outputs and $TMPDIR, where every t.TempDir() git
		// repository lands. `mdutil -s` only READS a volume's status -- nothing is changed
		// and no sudo is needed -- and it asks about the home work tree and TMPDIR, saying
		// off only when BOTH are disabled. mdutil -s prints "Indexing enabled." or "Indexing
		// and searching disabled."; "enabled" is not a substring of "disabled", so a
		// *disabled* case cannot be fooled by the enabled sentence. Missing or silent
		// mdutil reports on: unknown is not provisioned.
		{
