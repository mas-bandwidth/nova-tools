...
func Loop(in LoopInput) int {
	...
	for tick := 1; ; tick++ {
		fmt.Fprintln(in.Stdout, l.tick(tick))
...
// tick is one turn of the script's order, and it answers the one line the console keeps.
func (l *looper) tick(n int) string {
	var c loopCounts
	c.n = n

	// 2. RUN.
	runOut := l.step(func(w io.Writer) int { return l.runStep(w) })
	c.ran, c.harvested = fieldInt(runOut, "launched="), fieldInt(runOut, "harvested=")

	// 3. FILL.
	fillOut := l.step(func(w io.Writer) int { return l.fillStep(w) })
	...
	if strings.TrimSpace(l.in.Policy) != "" {
		mgrOut = l.step(func(w io.Writer) int { return l.managerStep(w) })
	}
...
type loopCounts struct {
	n, ran, filled, harvested, held, refused, dead int
}
...
func (c loopCounts) line(dry bool) string {
	line := fmt.Sprintf("LOOP TICK n=%d ran=%d filled=%d harvested=%d held=%d refused=%d dead=%d",
		c.n, c.ran, c.filled, c.harvested, c.held, c.refused, c.dead)
	if dry {
		line += " dry-run=yes"
	}
	return line
