...
type loopBench struct {
	queue, machines, lanes, root, policy string
}
...
func newLoopBench(t *testing.T) loopBench {
	...
	b.policy = filepath.Join(b.queue, "POLICY")
	if err := os.WriteFile(b.policy, []byte("floor=0\nwait-timeout=1s\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	return b
}
...
func TestLoopTickIsRunFillManagerInOrder(t *testing.T) {
	...
	want := "LOOP TICK n=1 ran=2 filled=3 harvested=4 held=2 refused=0 dead=0 failed=0"
...
// TestLoopDryRunWithAnInjectedRunStep: --dry-run is REFUSED when the run step is the wired
// one (TestLoopRefusesDryRunOnTheWiredRunStep, below, and the CLI's own refusal); a caller
// that injects its own RunStep owns that step's behaviour, and for it the flag reaches fill,
// takes no lock and moves nothing.
//
// This test used to be the whole dry-run guarantee, and it replaced RunStep with a no-op, so
// it never saw that the WIRED run step harvests, merges, reaps, cuts and launches under
// --dry-run. That is exactly why a regression test must not stub out the thing under test
// (Stella's cold read of #1430, defect 1).
func TestLoopDryRunWithAnInjectedRunStep(t *testing.T) {
	b := newLoopBench(t)
	now := time.Date(2026, 9, 18, 12, 0, 0, 0, time.UTC)
	launched := filepath.Join(b.queue, "launched")
	writeCard(t, launched, "card-001.md", "RESULT: CARD-1\n")
...
// TestLoopRefusesDryRunOnTheWiredRunStep: the run tick's six seams have no read-only mode
// and RunInput has no field to carry one, so a `loop --dry-run` that ran them would harvest,
// merge, reap, cut and launch on a queue a person pointed it at BECAUSE they were promised
// nothing would move. It is refused, and the refusal names the two verbs that do have one.
func TestLoopRefusesDryRunOnTheWiredRunStep(t *testing.T) {
	b := newLoopBench(t)
	var errb bytes.Buffer
	code := Loop(LoopInput{
		Queue: b.queue, Machines: b.machines, Lanes: b.lanes, Roots: b.root,
		Once: true, DryRun: true, Stdout: io.Discard, Stderr: &errb,
		Now: func() time.Time { return time.Date(2026, 9, 18, 12, 0, 0, 0, time.UTC) },
	})
	if code != 2 {
		t.Fatalf("loop --dry-run with the wired run step exited %d, want 2", code)
	}
	for _, want := range []string{"--dry-run", "fill --dry-run", "manager --dry-run"} {
		if !strings.Contains(errb.String(), want) {
			t.Fatalf("the refusal does not name %q: %q", want, errb.String())
		}
	}
	if _, err := os.Stat(filepath.Join(b.queue, "pulse.log")); !os.IsNotExist(err) {
		t.Fatalf("a refused loop still ran a tick: %v", err)
	}
}

// TestLoopValidatesEveryTableBeforeTheFirstTick: a path that is merely non-empty is not a
// registry. Each of these used to become a tick that placed nothing and said so only in the
// log -- or, for the lanes file, a refusal of every card in the queue, one at a time
// (Stella's cold read of #1430, defect 3).
func TestLoopValidatesEveryTableBeforeTheFirstTick(t *testing.T) {
	for _, c := range []struct{ name, flag string }{
		{"machines", "machines"}, {"lanes", "lanes"}, {"roots", "roots"}, {"policy", "policy"},
	} {
		t.Run(c.name, func(t *testing.T) {
			b := newLoopBench(t)
			in := LoopInput{
				Queue: b.queue, Machines: b.machines, Lanes: b.lanes, Roots: b.root,
				Policy: b.policy, Once: true, Stdout: io.Discard,
				Now:      func() time.Time { return time.Date(2026, 9, 18, 12, 0, 0, 0, time.UTC) },
				RunStep:  func(RunInput) int { t.Error("a refused loop ran a step"); return 0 },
				FillStep: func(FillInput) int { return 0 }, ManagerStep: func(ManagerInput) int { return 0 },
			}
			switch c.name {
			case "machines":
				in.Machines = filepath.Join(b.queue, "no-such-registry.tsv")
			case "lanes":
				in.Lanes = filepath.Join(b.queue, "no-such-lanes.tsv")
			case "roots":
				in.Roots = filepath.Join(b.queue, "no-such-root")
			case "policy":
				if err := os.WriteFile(b.policy, []byte("spawn-agents=true\n"), 0o644); err != nil {
					t.Fatal(err)
				}
			}
			var errb bytes.Buffer
			in.Stderr = &errb
			if code := Loop(in); code != 2 {
				t.Fatalf("a bad --%s exited %d, want 2", c.flag, code)
			}
			if !strings.Contains(errb.String(), "--"+c.flag) {
				t.Fatalf("the refusal does not name --%s: %q", c.flag, errb.String())
			}
		})
	}
}

// TestLoopStopsDependentStepsAndExitsNonZero: the loop threw every step's exit code away and
// answered 0, so a shift whose run tick could not run -- and whose fill and manager then
// placed cards behind a gate that never ran -- looked exactly like a shift with nothing to
// do (Stella's cold read of #1430, defect 3).
func TestLoopStopsDependentStepsAndExitsNonZero(t *testing.T) {
	b := newLoopBench(t)
	var ran []string
	var out bytes.Buffer
	code := Loop(LoopInput{
		Queue: b.queue, Machines: b.machines, Lanes: b.lanes, Roots: b.root,
		Policy: b.policy, Once: true, Stdout: &out, Stderr: io.Discard,
		Now:   func() time.Time { return time.Date(2026, 9, 18, 12, 0, 0, 0, time.UTC) },
		Sleep: func(time.Duration) {},
		RunStep: func(in RunInput) int {
			ran = append(ran, "run")
			fmt.Fprintln(in.Stderr, "RUN REFUSED: the queue state file is malformed")
			return 2
		},
		FillStep:    func(FillInput) int { ran = append(ran, "fill"); return 0 },
		ManagerStep: func(ManagerInput) int { ran = append(ran, "manager"); return 0 },
	})
	if code == 0 {
		t.Fatalf("a tick whose run step exited 2 answered 0")
	}
	if got := strings.Join(ran, ","); got != "run" {
		t.Fatalf("the tick ran %q after its run step failed, want run alone", got)
	}
	if !strings.Contains(out.String(), "failed=1") {
		t.Fatalf("the LOOP TICK line does not count the failed step: %q", out.String())
	}
	log, _ := os.ReadFile(filepath.Join(b.queue, "pulse.log"))
	for _, want := range []string{"LOOP STEP run exited 2", "LOOP SKIPPED fill, manager"} {
		if !strings.Contains(string(log), want) {
			t.Fatalf("pulse.log does not say %q: %q", want, log)
		}
	}
}

// TestLoopCountsAFailedFillAndStillRunsTheManager: a bench nobody can reach is no reason to
// stop merging what already came home. The failure is counted and the exit is non-zero.
func TestLoopCountsAFailedFillAndStillRunsTheManager(t *testing.T) {
	b := newLoopBench(t)
	var ran []string
	var out bytes.Buffer
	code := Loop(LoopInput{
		...
		Now:         func() time.Time { return time.Date(2026, 9, 18, 12, 0, 0, 0, time.UTC) },
		Sleep:       func(time.Duration) {},
		RunStep:     func(RunInput) int { ran = append(ran, "run"); return 0 },
		FillStep:    func(FillInput) int { ran = append(ran, "fill"); return 1 },
		ManagerStep: func(ManagerInput) int { ran = append(ran, "manager"); return 0 },
	})
	if code != 1 {
		t.Fatalf("loop exit = %d, want 1 when a step failed", code)
	}
	if got := strings.Join(ran, ","); got != "run,fill,manager" {
		t.Fatalf("a failed fill stopped the manager: %q", got)
	}
	if !strings.Contains(out.String(), "failed=1") {
		t.Fatalf("the LOOP TICK line does not count the failed fill: %q", out.String())
	}
}
