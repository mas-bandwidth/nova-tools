...
type CutInput struct {
	...
	// written: a locator that does not resolve (gh repo view non-zero) is refused with the
	// reason and no card is written, so a dead repo never spends admission plus the
