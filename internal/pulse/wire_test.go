...
func TestWiredOnceTickRunsEverySeam(t *testing.T) {
	...
	// 3. reap: the stuck runner was restarted, and the orphan card was disposed BY ITS
	// CAUSE (class I): this one left no harness evidence, so no rule reads it and it
	// leaves the loop as a triage packet -- never as another blind requeue.
	if len(restarter.names) != 1 {
		t.Errorf("restarted %v, want exactly one stuck runner", restarter.names)
	}
	if !strings.Contains(log, "triaged=1") || !strings.Contains(log, "requeued=0") {
		t.Errorf("the orphaned launched card was not disposed by its cause:\n%s", log)
	}
