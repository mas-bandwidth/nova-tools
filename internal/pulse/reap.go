...
func Reap(in ReapInput) int {
	...
	fmt.Fprintf(in.Stdout, "REAP roots=%d killed=%d locks=%d requeued=%d recut=%d probe=%d failed=%d triaged=%d temp=%d dry-run=%t\n",
		len(roots), killed, locks, d.requeued, d.recut, d.probe, d.failed, d.triaged, temp, in.DryRun)
	return 0
}
