...
// CardRow is one line of cards.tsv: the label, the slot (cut fills "-" until launch
// allocates one), the model route, and the path to the card text whose bytes become the
// batch's task.
type CardRow struct {
	Label string
	Slot  string
