...
// reader never has to open the file to learn what a refusal was about. `notes` is last
// because it is the only free-text column: everything a loop reads sits to its left.
func machineLine(m fleet.Machine) string {
	return fmt.Sprintf("MACHINE %s ssh=%s os=%s/%s roles=%s seat=%s cores=%d provider=%s mac=%s notes=%s",
		...
		oneline.Field(dash(m.Provider)), oneline.Field(m.MACField()),
