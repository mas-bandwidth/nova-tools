...
//
// The rule: markers live in their own directory beside the queue -- never in --ready --
...
// nothing else. A directory listing of --ready is cards, not cards-plus-markers.
func TestFillCountsCardsNotFilesAsQueueDepth(t *testing.T) {
	...
	if got := namesIn(t, ready); len(got) != 3 {
		t.Fatalf("the ready directory holds %v, want the three cards and no markers: queue depth is cards, not files", got)
	}
}
