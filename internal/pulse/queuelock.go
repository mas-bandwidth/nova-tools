...
//     cleared only when its taker is provably gone. AGE IS NOT DEATH.
...
// holder and a dead one -- with no process to spawn and no pid to guess.
var (
...
// closes it. So the removal is ATOMIC WITH THE VALIDATION, and the atom is `rename`. A
// recoverer first renames the record to a name only it knows, <path>.claim-<nonce>. Exactly
// one rename can succeed; every other recoverer gets ENOENT and backs off, having touched
// nothing. THEN the winner judges the record it is holding, which nobody else can reach, and
// unlinks it -- or, if the owner turns out to be alive after all, puts it back.
//
