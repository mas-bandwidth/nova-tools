...
func TestSeedTimeoutIsACouldNotRunAndNeverAKill(t *testing.T) {
	...
	ctx, cancel := expired()
	defer cancel()
	// nil is the seam's default form: exec.CommandContext, exactly what this test asked
	// for before the accept gate's wrapper existed (T03, #1648). The seam is the gate's
	// business, not the clock's, and a test about a dead deadline must not care which
	// builder made the command.
