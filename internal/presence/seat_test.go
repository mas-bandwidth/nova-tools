...
func TestOpenLogsInAsTheSeat(t *testing.T) {
	...
	if _, err := presence.Open(ctx, addr, ""); err == nil || !strings.Contains(err.Error(), "--seat") {
		t.Fatalf("Open with no seat and no password = %v; want a NOAUTH refusal naming --seat", err)
	}
