...
func TestEverySpecPulseSectionLinkResolves(t *testing.T) {
	...
	for _, name := range sections {
		path := filepath.Join(specPulseDir, name)
		...
		for i, line := range strings.Split(string(body), "\n") {
			for _, m := range specPulseLinkRe.FindAllStringSubmatch(line, -1) {
				...
				if _, err := os.Stat(resolved); err != nil {
					t.Errorf("%s:%d: link target %q resolves to %s, which does not exist; a section file's link is relative to the file it stands in, never to docs/SPEC-PULSE.md (#560), so %q must be re-based onto %s",
