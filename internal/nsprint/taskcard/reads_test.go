...
func TestReadPostScoreAdvancesThePrimary(t *testing.T) {
	...
	ciOK(c, head(0))
	calls := countCalls(c)
	out := post(t, c, 8300, "SCORE who=emma head="+head(0)+" score=9/10 gates=ci:ok,base:ok,scope:ok")
	if n := countCalls(c) - calls; n != 1 {
...
func TestUnderEightCutsAFixCopy(t *testing.T) {
	...
	line := "SCORE who=emma head=" + head(0) + " score=6/10 gates=ci:ok,base:ok,scope:ok: the test does not fail without the fix"
