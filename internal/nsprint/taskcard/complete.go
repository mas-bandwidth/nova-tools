...
const (
	RoutePro    = card.RoutePro
	RouteFlash  = card.RouteFlash
	RouteFriend = "friend"
)
...
// read as a `KEY: value` header line (card.KeyValue) whose key a card
...
// runner kind as it is, a classification kind by card.KindMap, and a build
// task (build, work, or no kind) as fix.
func ResultKind(kind string) (string, error) {
	switch {
	case typedrec.IsKind(kind):
		return kind, nil
	case kind == card.KindModel || kind == card.KindScript:
		return kind, nil
	case card.KindMap[kind] != "":
		return card.KindMap[kind], nil
...
// RenderBrief is the brief a friend's harness reads (friend serve's @brief),
// rendered from the same record as RenderHeader: what to build, where, the
// sentence that proves it, the PR body shape and the issue text. It needs
// nothing a friend route lacks: an absent field reads "-".
func RenderBrief(id string, rec map[string]string) ([]byte, error) {
	if len(rec) == 0 {
		return nil, &Refused{Why: "NOTASK " + Key(id)}
	}
	s := specOf(rec)
	...
	line := func(k, v string) { fmt.Fprintf(&b, "%s: %s\n", k, dash(v)) }
	line("TASK", id)
	line("TITLE", firstNonEmpty(s.Task, rec["title"]))
	line("STREAM", rec["stream"])
	line("ORIGIN", firstNonEmpty(rec["origin"], rec["ref"]))
	line("ROUTE", s.Route)
	line("KIND", s.Kind)
	line("REPO", s.Repo)
	line("BASE", s.Base)
	...
	line("EST", s.Est)
	line("DONE-WHEN", s.DoneWhen)
	b.WriteString("\nBuild the issue below as one PR against BASE. Write the DONE-WHEN test first (red at base-sha), then the change; run TEST and the tests of every package you touch. ")
	b.WriteString("The PR body carries, one per line: BASE, base-sha, PATHS, DEPENDS-ON, DONE-WHEN and STREAM as above, then the Closes line for ORIGIN. Never merge; no subagents.\n")
	b.WriteString("Write one typed line on stdout when you are done: DONE <PR number> <head sha>, or BLOCKED <why> when you cannot.\n\n---\n")
	b.WriteString(quote(dash(s.Body)))
	return []byte(b.String()), nil
}
...
// DealMove sends one waiting card to a friend's or a bench's ready set:
// friend:<To>:cards:ready (a bench is a seat like a friend).
type DealMove struct{ ID, To string }

// DealResult is one card's move, or its refusal (the move wrote nothing).
...
// Deal is the deal as one move per card: every move in one pipeline of the
// one move (ns_tcard_move id ready with the seat as its friend), one round
// trip however many cards. Nothing is generated: the bench's card is
// RenderHeader of the record and the friend's brief RenderBrief of it.
func Deal(ctx context.Context, c redis.Cmdable, by string, moves []DealMove) ([]DealResult, error) {
	pipe := c.Pipeline()
	cmds := make([]*redis.Cmd, len(moves))
	for i, m := range moves {
		o := Opts{By: by, Why: "deal", Friend: m.To, SetFriend: true}
		args := append([]any{m.ID, "ready", o.By, o.Why}, o.args()...)
		cmds[i] = pipe.FCall(ctx, FnMove, nil, args...)
	}
	if _, err := pipe.Exec(ctx); err != nil && !errors.Is(err, redis.Nil) {
		return nil, fmt.Errorf("%s: %w", FnMove, err)
	}
	out := make([]DealResult, len(moves))
	for i, cmd := range cmds {
		out[i].ID = moves[i].ID
		s, err := str(cmd.Val())
		if err != nil {
		...
		r, err := parseMove(FnMove, s)
		if why, ok := IsRefused(err); ok {
			out[i].Refused = why
			continue
		}
		if err != nil {
