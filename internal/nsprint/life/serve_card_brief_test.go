...
func TestServeBriefFromCompleteCard(t *testing.T) {
	...
	cfg.Models = map[string]string{"work": "test-model"}
	if _, err := life.ServeOnce(ctx, st, cfg); err != nil {
		t.Fatalf("serve once: %v", err)
	}
	// The brief sits in the child's card dir (cfg.Dir/cards/w1-<take ms>/).
	briefs, _ := filepath.Glob(filepath.Join(cfg.Dir, "cards", "w1-*", "brief.md"))
	if len(briefs) != 1 {
		t.Fatalf("briefs under %s = %v, want one", cfg.Dir, briefs)
	}
	b, err := os.ReadFile(briefs[0])
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{"CARD: w1\n", "STREAM: swarm: cards\n", "PATHS: internal/x/x.go\n",
