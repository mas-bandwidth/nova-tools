...
func TestL29c(t *testing.T) {
	...
	// 3. With down: Emma releases Stella's hold -> SUCCESS
	seq, err := land.CallRelease(f.ctx, f.client, f.sprint, "unit-l29c", "stella", "emma", "down-friend", "stella is down", "")
	if err != nil {
...
func TestL32(t *testing.T) {
	...
	// Initial state
	_, _ = land.ReconcileMirror(f.ctx, f.client, f.repo, gitRefs)

	// Ref moves in git
	gitRefs["refs/heads/card-3139"] = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
	missed, err := land.ReconcileMirror(f.ctx, f.client, f.repo, gitRefs)
	if err != nil {
