...
func TestFriendCopyRendersThePersonsBrief(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"work", []string{
			...
			"BEAT: friend:rowan's one beat loop (nova-sprint friend beat --as friend:rowan --loop, lease friend:rowan:beatloop) renews this copy's lease every second; " +
				"friend pull, card work and task take start it when none runs, so run no beat yourself;",
		...
		for _, never := range []string{"the wrapper", "never push", "never open a PR", "RESULT.md", "NO-SUBAGENTS", "card end", "--once", "your session's"} {
