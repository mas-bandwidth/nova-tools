...
func TestFriendCopyRendersThePersonsBrief(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"work", []string{
			...
			"END: nova-sprint friend done --as friend:rowan --ids p1~2 --ok --pr nova-tools#<n> --head <sha>",
			"nova-sprint friend done --as friend:rowan --ids p1~2 --fail '<why>'",
		...
		{"read", []string{
			...
			"\nDONE-WHEN: this copy is ended with the score of nova-tools#4300 at head " + strings.Repeat("cd", 20) + ": nova-sprint friend done --as friend:rowan --ids p1~2 --score N/10\n",
			...
			"END: nova-sprint friend done --as friend:rowan --ids p1~2 --score N/10 --gates ci:<green|red>,base:<ok|behind>,scope:<ok|over> --finding '<one line>'",
			"nova-sprint friend done --as friend:rowan --ids p1~2 --fail 'ABSTAIN <why>'",
		}},
		{"fix", []string{
