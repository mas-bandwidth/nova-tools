...
func TestReadCopyDealtToBenchRendersALintedCard(t *testing.T) {
	...
	if !strings.Contains(string(work), "FRIEND: friend:f owns this copy end to end") || !strings.Contains(string(work), "nova-sprint card end --id "+d[0].Copy+" --ok --pr nova-tools#<n> --head <sha>") {
