...
func TestReadCopyDealtToBenchRendersALintedCard(t *testing.T) {
	...
	if !strings.Contains(string(work), "FRIEND: friend:f owns this copy end to end") || !strings.Contains(string(work), "nova-sprint friend done --as friend:f --ids "+d[0].Copy+" --ok --pr nova-tools#<n> --head <sha>") {
