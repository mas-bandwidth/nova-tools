...
// idx set (sprint:<S>:idx:*) or roster (sprint:<S>:tasks), or a pointer
...
func TestTaskCardOneWriter(t *testing.T) {
	...
	binds := regexp.MustCompile(`local\s+(\w+)\s*=\s*'sprint:'\s*\.\.\s*\w+\s*\.\.\s*':idx:'`)
	...
	for _, sec := range sections[1:] {
		...
		for i, line := range strings.Split(sec, "\n") {
			...
			case strings.HasPrefix(target, "'task:'") && (m[1] == "HSET" || m[1] == "HSETNX" || m[1] == "HMSET" || m[1] == "HDEL") &&
