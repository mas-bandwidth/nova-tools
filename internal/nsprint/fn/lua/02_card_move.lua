...
function TM.finish(id, o)
  ...
  if leg == 'read' then
    if o.outcome == 'fail' then
      ...
      if score >= TM.PASS then
        ...
        -- a second read under TM.PASS is the author's fail (#4072)
  ...
  local mo = { by = o.by, why = why, ok = pok, copy = '', fields = pf, dry = o.dry, review = review ~= nil }
  ...
  if review then
    ...
    if review.demote then TM.demote(review.copy, 'read under ' .. TM.PASS .. ' twice: ' .. why, o.by, r[5]) end
  end
  local nxt = ''
  local err
  if cut_read then
