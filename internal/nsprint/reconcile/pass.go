...
type Counts struct {
	...
	// The blocked-resolve duty's moves (#3366): waiting -> ready once every
	// DEPENDS-ON landed, waiting -> parked once a parent never can.
	Released int
	Parked   int
}
...
func (c Counts) Line() string {
	return fmt.Sprintf("dealt=%d routed=%d expired=%d retried=%d ambiguous=%d reads=%d fixes=%d merging=%d carried=%d released=%d parked=%d",
		c.Dealt, c.Routed, c.Expired, c.Retried, c.Ambiguous, c.Reads, c.Fixes, c.Merging, c.Carried, c.Released, c.Parked)
}
