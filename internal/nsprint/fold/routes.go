...
// tally is one route's cards on one work type.
type tally struct {
	cards, prs, landed, useful, priced int
	micro                              int64
}

func (t *tally) add(pr, landed, useful, priced bool, micro int64) {
	t.cards++
	if pr {
		t.prs++
	}
...
func rankTypes(sprint string, byType map[string]map[string]*tally, p Policy) []*route.Fold {
	...
	for _, typ := range types {
		...
		for name, t := range byType[typ] {
			...
			r := route.FoldRoute{
				...
				Unpriced: t.cards - t.priced, Metric: per(t.micro, denom, t.priced), USD: tokens.Dash,
			}
			if t.priced > 0 {
