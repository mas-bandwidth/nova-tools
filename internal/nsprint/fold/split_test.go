...
func TestControl55Unknown(t *testing.T) {
	t.Run("card-with-no-end-record", func(t *testing.T) {
		...
		for _, want := range []string{
			"FOLD SPLIT sprint=" + fx.Sprint + " class=code type=fix route=fable cards=4 done=2 useful=2 prs=2 landed=2 closed=0 open=0 usd=1.3 usd_per_useful=0.65 usd_per_landed=0.65 unpriced=1\n",
			...
			"FOLD CLASS sprint=" + fx.Sprint + " class=code cards=10 done=7 useful=4 prs=7 landed=3 closed=1 open=3 usd=2.45 usd_per_useful=0.6125 usd_per_landed=0.816667 unpriced=2\n",
			"FOLD CLASS sprint=" + fx.Sprint + " class=read cards=3 done=3 useful=1 prs=0 landed=0 closed=0 open=0 usd=0.03 usd_per_useful=0.03 usd_per_landed=- unpriced=1\n",
