...
// xy/pitstop MGET (the tick is still the SCAN walk plus one pipeline).
func TestLiveNoBlockedLandedLines(t *testing.T) {
	...
	if len(mgets) != 1 {
		t.Fatalf("MGET count = %d, want the one xy/pitstop MGET", len(mgets))
	}
...
// when it is unset or empty. The key rides the existing xy MGET, so
...
func TestLiveTitlePitstop(t *testing.T) {
	...
	for _, c := range []struct {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			if mgets != 1 {
				t.Fatalf("MGET count = %d, want the one xy/pitstop MGET: %v", mgets, names)
			}
