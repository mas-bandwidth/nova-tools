...
// Fail writes the row and returns the batch in one ns_card_deal_fail call
// (#3322), as reconcile.DealFunctions does.
func (s *fnStore) Fail(ctx context.Context, fence, bench, state, why string, res []Reservation) (Failed, error) {
	args := []any{bench, fence, state, why, s.actor, ""}
	for _, r := range res {
		args = append(args, r.Card.Sprint, r.Card.Label, strconv.Itoa(r.Attempt))
	}
	reply, err := s.c.FCall(ctx, "ns_card_deal_fail", nil, args...).StringSlice()
	if err != nil {
		return Failed{}, err
	}
	if len(reply) > 0 && reply[0] == "FENCED" {
	...
	if len(reply) < 6 || reply[0] != "FAILED" {
		return Failed{}, fmt.Errorf("ns_card_deal_fail: %v", reply)
	}
	returned, _ := strconv.Atoi(reply[2])
	timeouts, _ := strconv.Atoi(reply[3])
	return Failed{Returned: returned, Timeouts: timeouts, State: reply[4], Hold: reply[5] == "1"}, nil
}
...
func countKind(msgs []redis.XMessage, kind string) int {
	n := 0
	for _, m := range msgs {
