...
//
// THE PAGE IS NEVER AUTHORITY. bin/fleet-state writes one Row per bench to
// queue/control/fleet-state.tsv and bin/fleet-live lists Live over the same
// decision; nothing in this tree reads that file back (TestNoReaderOfFleetStateTSV
// holds the grep). A projection read back is a snapshot pretending to be the
// present -- the keys are the present.
package state
...
// TTL. This is the production path both bins reach: Live filters on it and Row
// renders it.
...
// Live is what bin/fleet-live lists: the names whose heartbeat key still
...
// Row is one bench's fleet-state.tsv row as bin/fleet-state writes it:
