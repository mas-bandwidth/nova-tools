...
earlier poll of this run returned, so a backlog standing behind a slow turn is
counted once and not again on every poll while rule 17 holds the cursor, and
the first poll of a run, which has no earlier poll, and any poll that trips
rule 17's gap switch, which reads a window and not an arrival, being measured
by nothing, and entering neither the count nor the total; and the baseline
being the **mean** volume per poll over all of its polls since this state first
saw the conversation, **never taken below one**, held in this tool's state as
`flood:<conversation>` (a count, a total and the newest id seen, not a list of
polls; rule 4), and `-` until twenty polls exist, under which no flood blow
fires at all — and rule 11's stranger — and prints each blow with its reason.
The mean is over the whole life of the state and not a sliding window, because
a count and a total cannot drop the value leaving one, and the lifetime mean
lags in both directions: a room once busy and now quiet keeps a threshold too
high, which costs nothing, and a room once quiet and now busy is held at the
floor — `flood-multiple` messages per poll, the line's own number — until its
count catches up, which for a human-speed room whose polls are mostly empty is
most of the time; so `flood-multiple` is in practice the per-poll ceiling and
is chosen as one. The floor of one is the rest of that definition: a room
polled every thirty seconds is empty in most of its polls, so a baseline taken
as it falls is zero, `flood-multiple` times zero is zero, and the first
...
3. **`internal/chat/state.go`** — session ids, cursors, `turn:<id>` through `queued|firing|posting|delivered|uncertain`, `disclosed:`, `dm-channel:`, `backoff:`, `flood:<conversation>` (rule 20's baseline, a count, a total and the newest id seen), usage rows, at-most-one-pending-turn, `serve.lock` with the pid, every write through `.tmp` and rename. Tests: 4, 17, 23, 26, 27.
