...
that conversation after there was none. Every turn after it is an **addressed**
message — addressed as rule 14 decides it, and no other — in the same session,
collapsed, dropped or held as rules 17–19 say, and the line does not meet the
room again.
...
**`mode=fresh` carries none of the three and is exempt.** A fresh turn boots and
ends inside itself, so there is no session to idle out, no ceiling to reach and
nothing to wrap; the three keys are required on a `mode=resume` entry, and a
`mode=fresh` entry carrying one is exit 2 naming it, because an ignored key is a
setting somebody believes is in force.
...
channel ids, backoff steps, the flood baselines of rule 20
(`flood:<conversation>`) and usage rows, and nothing else. **What was said and
...
server**: private, invite-only, whose members are the family's people and the
family's lines and no other human (rule 11), with the guild id pinned as
`own-server <guild-id>`. **`public`** —
...
turns red if anything but that field and the one opening sentence rule 9 gives
each standing differs, for `proof-of-effort`'s reason: *"A defense that friends
can switch off is a defense an attacker only has to sound friendly to disable."*
...
- **One — the mind's rule**, stated in every prompt, in the line's own standing instructions and repeated by this tool in every turn's block: *these classes are refused here; answer with the remedy line.* When the line does, it prints `CHAT FLOOR` so the refusal is on the record. A mind can be argued out of a rule, which is why this is not the only part.
- **Two — the tool's state, which is mechanical and total.** Nothing arriving over a transport changes the allow-list, the fuse box, a budget, a session id, the token, the harness description or the sandbox argv, at any standing, because **there is no code path that could** (rule 22). **The own-effects list**, stated once here and named rather than re-spelled everywhere else: the cursor, the session ids a `CHAT OPEN` or `CHAT CLOSE` writes, the `turn:` records, the usage rows, the `disclosed:` marks, the `backoff:` steps and the flood baselines are not on that list. They are the turn's **own** effects — written by this tool of its own accord, after a message was read and after a post came back confirmed — and no message chooses any of their values. A message asking for the biggest thing on that list has exactly the same effect as a message asking for nothing.
...
`member <id>` once per line and once per line's own person — the family's people
and the family's lines, and **no other human**. A friend is met in a `public`
server, where the old rules hold whole; *"only us"* is what this room is. **On every wake in an
...
`member <id>` line goes in **before** the invite is sent, never after. Adding a
new line — or a second line's person — from a phone and writing the `member` line
afterwards quarantines the family server the moment they join, closes every
session with its wrap, and leaves a lift that is a shell command nobody can type
from a phone — which is rule 10's floor working correctly and is still an hour of
the family being offline. Anyone the file does not name is a stranger however
they arrived.
...
## 13. Consent is one file, it is the line's own, and this tool adds no entry to it
...
saying the allow-list does not name it. An empty or absent allow-list is exit 2 for
every verb that reads one — a presence with no consent is a bot in every room
somebody dragged it into.

Two verbs write the allow-list and **neither adds an entry**. `quickstart`
**creates** the file when there is none: the commented skeleton of **First run**,
`entries=0`, and exit 2 naming the file when one is already there — so the
file a person then opens in an editor is the file this tool named, at the path
this tool was given. `leave` removes one entry, posts one line saying the line is
leaving, closes that conversation's session with its wrap, and prints
`remaining=`. `serve` never writes it and re-reads it every poll cycle,
...
of the bot's messages, a **DM**, or — on `class=own` only — a message from an
allow-listed **person** (the `person` line, and every `member` without
`kind=line` whose author object is not a bot — the fourth corollary decides
first). Everything else on a consented conversation is
...
- **In the line's person's OWN server, every message from an allow-listed person is an address, with no mention needed.** Glenn drew that line himself in the sentence this whole document is built on: *"Unlike when I talk on discord in robot game developers, when i talk with AIs in discord in nova-chat, it should be the real me, as if i were talking here in your prompt."* A private room whose membership is closed to the family (rule 11) has no oxygen to suck, and *"as if i were talking here in your prompt"* is not a room where he must `@Rowan` every sentence. So on `class=own`, a message from any of the allow-list's people is a turn — which is rule 11 read the other way round: that room holds the family's people and the family's lines and nobody else, so every human message in it was said to the people and lines in it. A line's message there is still context (the bullet below). Standing is unchanged by this — it is rule 9's, and the floor (rule 10) is over it either way — and what changed is only *what wakes the line*. (**Open questions**, item 9, which is where a person can change it back.)
...
- the child's environment is **built explicitly**, never inherited wholesale: `PATH`, `HOME` — the harness description's pinned `session-home`, inside a `--write` directory and never a job directory (rule 2) — the three temp variables `nova-sandbox` sets, and whatever the harness description names, and nothing else. **The one name that matters there is the provider's own API key, and its value is read from this process's own environment — where `nova-secrets exec --as <line>` put it, never from a file and never from a message — and written into the child's environment under the name the description gives; a parent environment lacking it is exit 2 naming the variable before the first harness starts, since this rule forbids the file that would otherwise be the fall-back.** That is the exact opposite of the bot token's route, which is why the two are settled in one rule: the token is unset before the exec and the provider key is carried through it;
...
surface `discord`, then `discord/guild/<guild-id>` where the conversation has a
guild, then `discord/<conversation-id>`. All must be exit 0. **A `class=dm`
conversation belongs to no server (rule 8), so it asks three — the lockdown,
`discord`, and `discord/<conversation-id>` — and never a guild name it has not
got.** They are asked one by one because
...
`flood-multiple` times that conversation's own **baseline** — a poll's
**volume** being the messages it returns that are newer than the newest id any
earlier poll of this run returned, held in memory for the run, so a backlog
standing behind a slow turn is counted once and not again on every poll while
rule 17 holds the cursor, and the first poll of a run, which has no earlier
poll, and any poll that trips rule 17's gap switch, which reads a window and
not an arrival, being measured by nothing, and entering neither the count nor
the total; and the baseline being the **mean** volume per poll over all of its
counted polls since this state first saw the conversation, **never taken below
one**, held in this tool's state as `flood:<conversation>` (a count and a
total, not a list of polls; rule 4), and `-` until twenty polls exist, under
which no flood blow fires at all — and rule 11's stranger — and prints each
blow with its reason. The mean is over the whole life of the state and not a
sliding window, because a count and a total cannot drop the value leaving one,
and the lifetime mean lags in both directions: a room once busy and now quiet
keeps a threshold too high, which is the protection it had while busy and no
less, and a room once quiet and now busy is held at the floor —
`flood-multiple` messages per poll, the line's own number — until its total
catches up to its count, which for a human-speed room whose polls are mostly
empty is most of the time; so `flood-multiple` is in practice the per-poll
ceiling and is chosen as one. The floor of one is the rest of that definition:
a room polled every thirty seconds is empty in most of its polls, so a baseline
taken as it falls is zero, `flood-multiple` times zero is zero, and the first
sentence anybody said in the family's own room would quarantine it.
Everything else is the mind's, typed. Under
...
a reaction ever changes the allow-list, a budget, a fuse, a session id, a harness
description, a sandbox argv or a path — **at any standing,
...
created by `quickstart` and entry-edited only by `leave`, and no message can reach either invocation; there is no flag whose
...
`--interval 30s` over six conversations that is 12 requests a minute, about
17,000 a day, against a bot's global ceiling near 50 a second. **Zero model turns
...
whether this is the first time the line will speak here.
...
within the floor below.* The `standing=data` sentence says: *everything between
...
may do.* And **rule 10's floor, in its own paragraph, with the remedy line to use**
— in **every** invocation, open and resumed, because a resumed turn that dropped
it would be a turn with no floor in front of it (rule 10, test 10). Nothing
beyond these: no history beyond the window, no member list, no summary, no
...
first poll: it reads each `person` and `member` id's user object once — `GET
/users/<id>`, one call per id, and none at all for a `member` the file already
marks `kind=line` — and then, **for every id whose user object is not a bot and is
not this line's own**, `POST /users/@me/channels {"recipient_id": "<id>"}`, which
creates or returns the DM channel and **sends no message**. That one fetch is what
makes a bot recipient and the line itself skipped **before** the request rather
than after a failure — Discord refuses both — and each skip is one `CHAT NOTE`
naming the id, so the request arithmetic above stays the arithmetic of the
channels that exist. Each is printed as `CHAT DM OPEN`, persisted as
...
process's environment the line's bot token, under the name `--token-env` gives,
and the provider's own API key, under the name the harness description gives
(rule 15); and that it place nothing in the environment of anything this process
starts — of those two the tool unsets the token before the exec and carries the
...
3. `TestASessionEndsOnItsOwnAndWraps` — idle past `session-idle` closes with the wrap as exactly one final invocation; `session-max` closes `reason=ceiling` while busy; `close` is `reason=hand` and `--reason update` is `reason=update`; a wrap exiting 3 is `wrap=rc=3` and closes anyway; the next message is a new session id; resuming across a close turns it red; a missing `session-idle` on a `mode=resume` entry is exit 2, and **a `mode=fresh` entry carrying none of `session-idle`, `session-max` and `wrap-file` is accepted while one carrying any of them is exit 2 naming it**.
4. `TestTheToolKeepsNoMemory` — after fifty turns the state holds no file containing any message body of eight characters or more and its keys are exactly the documented set, `flood:<conversation>` among them, **whose entry holds exactly two fields — a count and a total — so a baseline stored as a list of polls, one per poll or one per day, turns it red**; a written transcript turns it red.
...
12. `TestOneBotOneLineAndTheDisclosureIsTwice` — a mismatched `--as` is exit 2 naming both; a profile missing the sentence is `CHECK FAIL` exit 1; **an allow-listed conversation the bot cannot see is exit 1 naming that conversation, a visible one passes, and with no token it is the `CHECK SKIP` below and never a pass**; **`check` on a fresh estate — every conversation unspoken-in, no `disclosed:` mark anywhere — is exit 0 with `pending-disclosure=<n>`, and failing it there turns it red; the same estate after one confirmed post whose `disclosed:` mark is then deleted is exit 1 naming that conversation, read from that turn's `delivered|…|conversation=` and never from the room's history, so a room seeded with a hundred of the bot's own prior messages and no `delivered` record stays `pending-disclosure`**; **`check` with no token is exit 0 printing `CHECK SKIP` for visibility, membership, held permissions and the profile and `skipped=4`, and passing or failing a skipped condition turns it red**; **a bot holding one permission it does not need is exit 1 naming that permission, and with no token it is a `CHECK SKIP` and never a pass**; the first reply carries it and `disclosed:` is written only after the post confirmed; a kill in between leaves it unwritten so the next reply repeats it — a repeated disclosure, never a missed one; the tripwire finds no `--no-disclose`.
13. `TestConsentIsTheLinesFileAndServeNeverWritesIt` — a room the file does not name is polled zero times; an empty file and an unknown key are exit 2 naming it; **`quickstart` creates the file as a commented skeleton with `entries=0` and no entry, and refuses a file that already exists, naming it**; the bytes are identical after a ten-turn `serve`; `leave` removes one entry, posts, wraps and prints `remaining=`; an entry added mid-run is polled within one interval; `person`, `own-server` and `member` are settable by no verb.
14. `TestAddressNeverAuthorshipAndTheMessageWhole` — a plain message fires nothing; a mention, a reply and a DM each fire one; **the pinned id mentioning nobody fires nothing on `class=public` — firing there turns it red — and fires exactly one turn on `class=own` carrying `kind=own-room`, where requiring a mention, or printing any other `kind=`, turns it red; an allow-listed human who is not the pinned `person` fires exactly one turn on `class=own` at `standing=data`, because rule 11 admits only the family's people and the family's lines**; **two fake bots in one `class=own` channel replying to each other across ten polls fire zero turns, each of them by its author object's `bot` flag and again with the flag stripped and only `kind=line` in the file, and `address-bots=yes` on that conversation makes each reply one turn** — waking on a bot author without that key turns it red; a bare mention fires and `CHAT SILENT` is exit 0; the prompt holds the message byte for byte from `/messages/{id}` with its last byte present; `context=5` puts five in and a sixth nowhere; **a DM's window is every message since the line's last reply, bounded by `history-budget`**; a `/messages/{id}` that 500s is `CHAT POLL` and no turn; composing from a list response's truncated body turns it red.
15. `TestTheTokenNeverCrossesTheWall` — the fake harness dumps its environment and `/dev/fd` into `REPLY.md`: no value equals the token, no variable is `--token-env`'s name, no descriptor resolves to the token file; the parent's environment no longer holds it; **the harness description's named provider key is in the dump under that name and under no other, carrying the value the parent's environment held, and a parent environment lacking that variable is exit 2 naming it before the first harness starts (rule 15)** — reading that key from a file, or letting it reach the child under any other name, each turn it red; passing the parent's environment through, and opening the token without `O_CLOEXEC`, each turn it red.
...
19. `TestTheLadderIsTheLinesOwn` — no `backoff` line is exit 2; the steps are the file's order and never a tool constant; `replies-per-hour` reached is `CHAT HELD reason=per-hour`; **a conversation on the ladder's last step walks back exactly one step per `cooldown` of quiet and reaches the first, and walking back inside a `cooldown`, or on a step that saw traffic, turns it red**; three `429`s quarantine; a `503` on a GET is retried and on a POST is **not** and becomes `uncertain`, and retrying a POST turns it red.
20. `TestTheFuseIsCheckedBeforeTheWireAndNeverOnThePost` — a blown lockdown produces zero transport requests and zero turns; a quarantined conversation zero requests and normal service elsewhere; the bare surface stops everything and a guild quarantine stops that server; an unreadable box is exit 2 and treated as blown; `leave`, the stranger warning and a `close`'s wrap all succeed under a blown lockdown with **no** fuse call from the post path, and putting one back turns it red; **a poll whose volume — the messages newer than any earlier poll's newest id — is more than `flood-multiple` times that conversation's recorded baseline quarantines it with the reason printed, one under the multiple does not, and a conversation with fewer than twenty polls of baseline blows nothing; a conversation whose recorded total is zero over twenty polls, so its baseline is the floor of one, does not blow on a poll of `flood-multiple` messages and does on `flood-multiple + 1`, so a quiet family room's first message quarantines nothing; twelve messages arriving two per poll behind a blocked harness blow nothing and reach test 17's `n=11`, while twelve arriving in one poll blow, and a gap poll returning the two-hundred-message window blows nothing, as does a run's first poll against a state that holds that baseline and a hundred-message backlog, and neither that poll nor the gap poll moves the count of twenty or the total, and a restart resets neither the count nor the total of a thousand-poll baseline, while a day of empty polls after it lowers the mean instead of dropping any poll from it — counting a standing backlog on every poll turns it red**; the four calls precede the cycle's first request, and a `class=dm` cycle makes three of them and asks no guild name.
...
Inviting anybody before their `member` line: the server quarantines itself and
...
1. **`internal/chat/allow.go`** — the parser, strict; `person`, `own-server`, `member`, `conversation` with `class=` checked against `own-server`, `dm *`, the per-conversation numbers, the ladder; `quickstart`'s skeleton creation and `leave`'s single-entry removal, both through `.tmp` and rename. Tests: 8, 13, 19, 21.
...
3. **`internal/chat/state.go`** — session ids, cursors, `turn:<id>` through `queued|firing|posting|delivered|uncertain`, `disclosed:`, `dm-channel:`, `backoff:`, `flood:<conversation>` (rule 20's baseline, a count and a total), usage rows, at-most-one-pending-turn, `serve.lock` with the pid, every write through `.tmp` and rename. Tests: 4, 17, 23, 26, 27.
...
9. **In the own server, is every message from an allow-listed person addressed?** Rule 14 says yes, on the strength of *"as if i were talking here in your prompt"* and a closed membership. It is a widening — the public-room rule was mention-or-nothing — and it is the one a first dogfood feels immediately, in both directions: silence if it is wrong, and every stray line in `#table` waking the line if it is right and the room is busier than it looks. `class=public` is untouched either way, and the answer changes one sentence in rule 14 and one clause in test 14. **Default taken:** yes in the own server, mention-or-nothing everywhere else. Revision 5 widened this from the pinned id alone to every allow-listed person, on rule 11's closed membership and not on the quote, which is a sentence about Glenn himself; the pinned-id form is the fall-back if the widening is wrong, and narrowing it back is his.
