...
`flood-multiple` times that conversation's own **baseline** — a poll's **volume**
being the messages it returns that are newer than the newest id the previous poll
of this run returned, so a backlog standing behind a slow turn is counted once and
not again on every poll while rule 17 holds the cursor, and the first poll of a
run, which has no previous poll, and any poll that trips rule 17's gap switch,
which reads a window and not an arrival, being measured by nothing; and the
baseline being the **mean** volume per poll over all of its polls since this state
first saw the conversation, **never taken below one**, held in this tool's state
as `flood:<conversation>` (a count, a total and the newest id seen, not a list of
polls; rule 4), and `-` until twenty polls
exist, under which no flood blow fires at all — and rule 11's stranger — and
prints each blow with its reason. The mean is over the whole life of the state and
not a sliding window, because a count and a total cannot drop the value leaving
one, and the lifetime mean fails only leniently: a room once busy and now quiet
keeps a threshold too high, never one too low. The floor of one is the rest of
that definition: a room polled every thirty seconds is empty in most of its polls,
so a baseline taken as it falls is zero, `flood-multiple` times zero is zero, and
the first sentence anybody said in the family's own room would quarantine it.
...
20. `TestTheFuseIsCheckedBeforeTheWireAndNeverOnThePost` — a blown lockdown produces zero transport requests and zero turns; a quarantined conversation zero requests and normal service elsewhere; the bare surface stops everything and a guild quarantine stops that server; an unreadable box is exit 2 and treated as blown; `leave`, the stranger warning and a `close`'s wrap all succeed under a blown lockdown with **no** fuse call from the post path, and putting one back turns it red; **a poll whose volume — the messages newer than the previous poll's newest id — is more than `flood-multiple` times that conversation's recorded baseline quarantines it with the reason printed, one under the multiple does not, and a conversation with fewer than twenty polls of baseline blows nothing; a conversation whose recorded total is zero over twenty polls, so its baseline is the floor of one, does not blow on a poll of `flood-multiple` messages and does on `flood-multiple + 1`, so a quiet family room's first message quarantines nothing; twelve messages arriving two per poll behind a blocked harness blow nothing and reach test 17's `n=11`, while twelve arriving in one poll blow, and a gap poll returning the two-hundred-message window blows nothing, as does a run's first poll — counting a standing backlog on every poll turns it red**; the four calls precede the cycle's first request, and a `class=dm` cycle makes three of them and asks no guild name.
