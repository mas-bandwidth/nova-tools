...
- the child's environment is **built explicitly**, never inherited wholesale: `PATH`, `HOME` — the harness description's pinned `session-home`, inside a `--write` directory and never a job directory (rule 2) — the three temp variables `nova-sandbox` sets, and whatever the harness description names, and nothing else. **The one name that matters there is the provider's own API key, and its value is read from this process's own environment — where `nova-secrets exec --as <line>` put it, never from a file and never from a message — and written into the child's environment under the name the description gives; a parent environment lacking it is exit 2 naming the variable before the first harness starts, since rule 15 forbids the file that would otherwise be the fall-back.** That is the exact opposite of the bot token's route, which is why the two are settled in one rule: the token is unset before the exec and the provider key is carried through it;
...
`flood-multiple` times that conversation's own **baseline** — the **mean** message
count per poll over its own last twenty-four hours of polls, **never taken below
one**, held in this tool's state as `flood:<conversation>` (a count and a total,
not a list of polls; rule 4), and `-` until twenty polls exist, under which no
flood blow fires at all — and rule 11's stranger — and prints each blow with its
reason. The floor of one is the whole of that definition: a room polled every
thirty seconds is empty in most of its polls, so a baseline taken as it falls is
zero, `flood-multiple` times zero is zero, and the first sentence anybody said in
the family's own room would quarantine it. Everything else is the mind's, typed.
Under
...
20. `TestTheFuseIsCheckedBeforeTheWireAndNeverOnThePost` — a blown lockdown produces zero transport requests and zero turns; a quarantined conversation zero requests and normal service elsewhere; the bare surface stops everything and a guild quarantine stops that server; an unreadable box is exit 2 and treated as blown; `leave`, the stranger warning and a `close`'s wrap all succeed under a blown lockdown with **no** fuse call from the post path, and putting one back turns it red; **a poll returning more than `flood-multiple` times that conversation's recorded baseline quarantines it with the reason printed, one under the multiple does not, and a conversation with fewer than twenty polls of baseline blows nothing; a conversation whose recorded mean is at the floor of one does not blow on a poll of `flood-multiple` messages and does on `flood-multiple + 1`, so a quiet family room's first message quarantines nothing**; the four calls precede the cycle's first request, and a `class=dm` cycle makes three of them and asks no guild name.
