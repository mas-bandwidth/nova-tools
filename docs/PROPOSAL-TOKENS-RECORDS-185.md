...
// Existing unsupported-subfield diagnostics (e.g. unreadable turn ID) are counted in Shapes alongside retained records (count = 1). Retained; spendability follows the existing mapping/identity/usage predicate and is not changed solely by this diagnostic.
...
* **Separation of Retained Observations from Refusals / Truncation**:
  - **Count 1 vs Count 0**:
    ...
    * *1 Retained Observation Emitted (Count = 1)*: Records with unavailable supported counters (`Presence == "unavailable"`, completeness gap), arithmetic mismatches (`input + output != total`), or impossible subsets (`cached_input > input`, `reasoning > output`) parse successfully into sealed `nova.tokens.observation/2` envelopes, emit 1 observation (`count = 1`), retain their original sealed content IDs and mapping diagnoses, are retained in shard records, and are marked `Spendable = false`. For unsupported subfields (such as an unreadable turn ID in `internal/tokens/codex.go:586–592`, which counts `token_usage_record_with_unreadable_turn_id` in `Unsupported` while omitting the subfield, verified in `codex_test.go:649–659`), the observation parses validly into a sealed envelope and is retained in shard records (`count = 1`). Retained; spendability follows the existing mapping/identity/usage predicate and is not changed solely by this diagnostic.
  - **Strict Counter Separation**:
    Do NOT increment wire-shape (`Unsupported`) or boundary refusal (`Refusals`) counters solely because a coverage reason uses `unsupported_rows`. Those counters strictly track unmapped source shapes, syntax refusals, and specific unsupported-subfield diagnostics (which accompany retained records with `count = 1`; Retained; spendability follows the existing mapping/identity/usage predicate and is not changed solely by this diagnostic). Coverage reason mapping alone never increments wire counters.
...
| Unsupported subfield with valid enclosing record (e.g. unreadable turn ID) | 1 (retained) | `unsupported_rows` | Specific `source_id` | Counted in `Unsupported` (`token_usage_record_with_unreadable_turn_id`), subfield omitted, record retained in shard (`count = 1`). Retained; spendability follows the existing mapping/identity/usage predicate and is not changed solely by this diagnostic. |
...
2. **One Retained Observation Emitted (Count = 1)**:
   - Records with unavailable supported counters, arithmetic mismatches, or impossible subsets parse validly into sealed `nova.tokens.observation/2` envelopes, emit 1 observation (`count = 1`, contributing to `record_count`), retain their original sealed envelope IDs and mapping diagnoses in shard records, and are marked `Spendable = false` due to incompleteness or internal arithmetic/subset inconsistency.
   - For unsupported subfields (such as unreadable turn IDs in `internal/tokens/codex.go:586–592`, which counts `token_usage_record_with_unreadable_turn_id` in `Unsupported` while omitting the subfield, verified in `codex_test.go:649–659`), the observation parses validly into a sealed envelope and is retained in shard records (`count = 1`). Retained; spendability follows the existing mapping/identity/usage predicate and is not changed solely by this diagnostic.
...
3. **Shard & Inventory Verification**:
   ...
   - **Strict Observation Validation on Every Shard Line**:
     ...
     - **Retained Unsupported-Subfield Witness**:
       Existing decoder logic (`internal/tokens/codex.go:586–592`, tested in `codex_test.go:649–659`) counts unsupported subfields (such as `token_usage_record_with_unreadable_turn_id`) in `Unsupported` while omitting the invalid subfield and retaining the observation (`count = 1`, sealed identity intact; Retained; spendability follows the existing mapping/identity/usage predicate and is not changed solely by this diagnostic; for otherwise-valid unique `resp-t2`, `Spendable = true`). Shard validation accepts these retained observations as valid `nova.tokens.observation/2` envelopes without imposing spendability; an unsupported subfield diagnostic on the decoder side does not invalidate or exclude the sealed observation envelope from shard records.
...
| `TC-VAL-17` | `retained_unreadable_turn_id_observation_valid` (TC-VAL-20b) | Retained Obs / Subfield | Shard contains observation decoded with unreadable turn ID (otherwise-valid unique `resp-t2`, `codexShapeNoTurnIDLexeme` counted in decoder `Unsupported`; `turn_id` omitted from receipt; `codex.go:586–592`, `codex_test.go:649–659`). | Observation envelope is valid `observation/2`, passes `ValidateInstalledDirectory` as retained record (`count = 1`, `Spendable = true`). Retained; spendability follows the existing mapping/identity/usage predicate and is not changed solely by this diagnostic. Preserves sealed identity, omitted `turn_id`, `Unsupported` increment, and source coverage reason (`unsupported_rows`). |
