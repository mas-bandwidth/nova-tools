...
What no other section carried, and this one does: **the coordinator's notes**, which manager 1
reads before it decides and manager 2 reads before it writes a brief; **the six gates** a delegation
...
and partial child success never closes the parent (manager 5). Rules 7 and 8, promotion and
...
nothing (manager 5). **(Rowan's decision, for review, on ideas#778)**: the walls are strongest at the
