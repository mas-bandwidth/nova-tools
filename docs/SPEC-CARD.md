...
All ten cutter kinds have one reviewed worked example in [Card exemplars](EXEMPLARS.md). A cut
card links its matching row in the task body as `Example to follow:`; that prose is guidance and
never a second command authority. The catalog says which six kinds use `RenderCardV2` today and
which four retain their legacy renderer, so the example cannot be mistaken for renderer support.
