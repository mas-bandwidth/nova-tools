...
| 4 | admission: `swarm.cardShapeFailure` | `internal/swarm/batch.go:2198` | for `opencode/` and `deepseek/` models: a line beginning `STEP 1` in the **first 15 lines** (`hasStep1`, `:2269`; the bound is unchanged, SPEC-TOOLWORK section 5 rule 1 orders the header so STEP 1 fits it, #1728), no all-capitals first line, no "launcher" in lines 1-3, a fourth `STEP` only under `MODE: explore`; the body is `swarm.CardShapeRefusal` (`:2212`), which the cutter runs on every card before writing it (`internal/pulse/cut_template.go:1203`) |
