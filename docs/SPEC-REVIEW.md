...
A section's body names its rule on a `rule: <id>` line, defaulting to the heading text, and states each required witness on a `positive: <glob>` or `negative: <glob>` line; the glob is matched against the path of every line the diff adds, the first added line in a matching file is that witness's `file:line`, and every listed witness must still be held at the head.
