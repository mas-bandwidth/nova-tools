...
| `model.id` / `model.basis` | `primaryModelId`, the reported harness identifier, is the ONLY source of `model.id`. A non-empty primary compatible with the split (no split, or a single split entry that is the primary) is that ID with `harness_reported`. No primary is `{id: null, basis: unknown}` even when `modelUsage` holds a single split entry, which is retained in `model_usage` and never inferred into `model.id`. More than one split model, or a non-empty primary that does not match the single split entry, is `{id: null, basis: mixed}`. `model_basis` and `requested_model` are never emitted as wire keys. |
