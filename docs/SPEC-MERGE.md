...
**`status`.** `status` reads `queue.json` and `hold` and NOTHING ELSE — no host, no clone, no lock — and prints the order, the two sets and the hold: `QUEUE ENTRY pos=<n> entry=<pr> state=queued`, `QUEUE ENTRY pos=- entry=<pr> state=skipped|parked`, and `QUEUE STATUS lane=<dir> queued=<n> skipped=<n> parked=<n> hold=<reason|-> by=<who|->`. A report that needs a token is a report nobody runs on the bench they are standing at.

**`hold <reason> --who <name>` and `release`.** `hold` writes the reason into `<lane>/hold`; while it exists the sweep, every enqueue (`add`, `front`, `react`) AND `run` ITSELF refuse at exit 2 with the remedy `nova-merge queue release  # <reason>`, so a sweep can never re-enqueue over a coordinator's hold and a pass can never land over one either. `run` reads it at the top of every pass, so a hold written during a `--loop` stops the next one. `release` removes the file and prints how long it stood. An empty reason is refused: a hold nobody can read is not a hold. `--who` is REQUIRED — the person the hold belongs to is a fact this tool refuses to guess (rule 20), and this binary reads no environment variable, so `$USER` is the caller's to pass.
...
```
QUEUE ENTRY pos=<n|-> entry=<pr> state=<queued|skipped|parked>
...
**The landing is a verb (2026-09-18).** The seven steps after `BATCH OK` ran **eight times that day**, by hand or by a child, and they were the same seven every time: push the branch, open the batch's pull request, wait for its `ci-ok`, rerun the two known flakes when they are what went red, enqueue at the front, watch the queue to the merge commit, close every member with a pointer to the batch that carried it. A procedure run eight times by hand is a verb nobody has written yet (Glenn, 2026-09-17: *everything sketched becomes a tool*).

```
nova-merge batch … --land [--flakes <file>] [--max-rounds <n>] [--interval <duration>]
```

It is a **flag on the gate rather than a verb of its own**, because it is the same run: the evidence it hands the one door is the `BATCH OK` line printed a second earlier, over the tree in this clone. A second verb would have to be handed that line, that branch and that head, and a caller who mistyped any of the three would land a tree nobody gated.

Five things in it are rules rather than conveniences.

- **The push is rule 21 when the branch already exists.** A batch rebuilt under its own name is published by a compare-and-swap on the head this run READ, so a branch somebody else moved is a refusal. A branch the forge does not have at all is an ordinary push, and a branch already at this commit is nothing.
- **Both greens are asked, as `land` asks them.** The gate's green is a bench's, on one operating system; CI's is the forge's, on three. Step 3 waits for the pull request's own `ci-ok` before step 5 reaches the door, and the door asks again.
- **A rerun is one round, and every failing test or none.** `--flakes` is a shrink-only list of `<package> <Test> <reason>` lines; `tools/ci/flakes.txt` ships with the two the fleet had that day. A job that failed and named NO test is never a flake — a rerun of an unexplained failure is a coin flipped on a landing — and a test that fails twice is a red tree, whatever the list says. `internal/ci`'s `TestTheFlakeListOnlyShrinks` holds the ceiling from the other side: adding a line is a decision for a person, and the way to take one off is to fix the test.
- **A dequeue is read, never guessed.** Only a merge group whose non-success jobs are all **cancelled darwin shards** earns one re-enqueue: that is the fleet's two darwin runners being busy behind another group. A failed merge-group job is a red tree on the base it would land on, and stops with the failing lines printed.
- **A close that fails is not a failed landing**, and a member the gate DROPPED is never closed. The first would send the next person to land a batch that is already in; the second would close a pull request whose work never landed, which is the worst failure available here.

The forge seam is `merge.LandForge` — open or update the pull request, read a run's failing jobs and their test names, rerun the failed jobs, read a merge-queue entry, close a member — and **it holds no enqueue and no merge primitive of any kind**, for the reason the lock exists: a seam that cannot enqueue cannot be talked into enqueueing. Every test of the landing drives `merge.FakeHost` and none opens a socket; the poll is `Deps.Sleep` and the deadline is `Deps.Now`, so none of them waits either.
...
**Every other site moved onto it or went.** `nova-merge sweep`'s host now offers a green pull request to the one door instead of enqueueing it, and prints `refused=<n>` for the ones the door would not take; `nova-pulse sweep`'s `GHEnqueuer` — the site that ran `gh pr merge --auto` — is now a thin adapter that reads the head and offers it to the door; `.github/scripts/revert-on-red.sh` opens its revert pull request and LEAVES IT OPEN with a notice, enabling no auto-merge. `internal/ci`'s reactor reaches no forge either: what it enqueues is this session's green list, and that list is the LANE'S `queue.json` — the file `nova-merge queue` writes, `queue status` prints and `run` walks — rather than the redis set `merge:queue`, which nothing in this tree ever read (edge 17 below). One entrance to the forge's queue, one list of what the next batch is built from, and each is a thing somebody can look at.
...
## The gate on a bench, the forge here (2026-09-18)

**The two halves of a batch want two different machines.** The clone, the merges and the suite want cores and a toolchain — hulk is 64 of them against the Studio's 32, which are also Glenn's, and the fleet's Go and sbcl live under `~/sdk` there. Everything that touches the forge wants a credential, and a bench holds **none**: secrets are sealed once in the store and never copied between machines, so on a bench `--plan`, `checks=required` and `--land` all refuse. Every landing child that day ran the gate over ssh by hand, read the `BATCH OK` line off a terminal, and did the forge half from the Studio. **Eight times.** A procedure run eight times by hand is a verb nobody has written yet.

```
nova-merge batch --on <machine> --local-root <dir> [--machines <file>] …
```

**The split is exactly the one the hand made.** On the machine: the clone, the merges in the order given, `build`, `vet`, `vet-windows`, `test`, `lisp`, and the bundle. On this machine: every member's own `ci-ok`, the plan, the push, the pull request, the one door, the queue watch and the members' closes. The seam is `batchSite` (cmd/nova-merge/batchsite.go) and **there is no method on it that can reach a pull request**: a seam that could do both halves is a seam somebody eventually asks a bench to do the forge half through.

Six things in it are rules rather than conveniences.

- **The machine is a registry row, checked on the NAME before any ssh.** `internal/fleet.RequireBench` answers whether the name is a machine at all and whether it may take work — the lock of 2026-09-18: a runner host is CI-only, because a gate beside the merge group's shards makes the shards slow, the gate red and the queue stop. A refused machine is never connected to. The ssh target is that row's, so nothing here guesses a hostname.
- **`--root` is a path on the OTHER machine**, and a leading `~/` crosses the seam unexpanded, because the home it names is the bench's rather than this one's. It is held to a strict shape — absolute or `~/`, at least two elements deep, no `..`, and letters, digits, dot, dash, underscore and slash — because the gate REMOVES its own working directory there before it rebuilds it (Glenn, 2026-09-17: *it is just one mistake away from deleting the whole disk*). `--local-root` is the root on this machine and is **required** with `--on`: a batch built elsewhere has to land somewhere here.
- **The machine's own toolchain is what builds.** Every step runs under one prelude that puts `~/sdk/*/bin` in front of `PATH`, newest by name — the bench provisioning standard's toolchain roots (#1364/#1419: `sdk` is the one home root carrying execute) — and unsets `GOFLAGS` and the `GOTEST*` family, which is `internal/goenv`'s `Clean` written as the shell. A non-interactive ssh gets whatever the machine's rc file happens to export, and a gate that built against the distribution's go1.22 because a login shell was not involved is a gate that refuses a tree CI builds. Edge 1's toolchain check asks **that** machine's `go`, under `GOTOOLCHAIN=local` so the checkout's own `go.mod` cannot make the probe unanswerable.
- **The batch crosses back as a git bundle** — the batch's own commits, prerequisite the base the gate started from — which is the file the landing children carried by hand. It comes over on **every** green gate and not only before a landing: a batch that cannot cross the seam is a batch nobody can push, and the run that learns that should be the run that built it. What came back is checked to be a bundle **at the seam**: a `cat` over ssh answers with whatever the far side wrote on its stdout, and every wrong answer is otherwise a failure ten minutes later about the wrong thing.
- **`--land` pushes from here.** It clones on this machine, feeds the bundle in, and runs the same seven steps over that clone, so nothing in the landing can tell the tree was built somewhere else — and the bench is never asked to push, which is what the tests assert.
- **`--on` and `--plan` do not go together.** The plan asks git which diffs go together and runs no step of the suite, so it needs no bench: run the plan here and the gate it prints `--on` the machine.

**Every line says where.** Each progress line and the verdict carry `on=<machine>` — `on=local` for a gate this machine ran — because the green says this tree built, vetted, tested and ran the lisp suite **on one machine**, and which machine is part of what was checked. `ParseBatchReceipt` reads the line as fields, so the receipt the one door takes is unchanged.

**The ssh seam is `internal/merge.Remote`: two calls, run a script and fetch a file.** The script is built by `merge.RemoteScript` rather than inside the implementation, so the fake bench the tests drive runs **the same text** a real bench runs — a seam that let the test build its own script would prove nothing about the one that ships. No test opens a connection: the fake answers those scripts against `/bin/sh` and the bare fixture repository, so the clone, the merges, the conflict, the suite and the bundle are all really performed.
