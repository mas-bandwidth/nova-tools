...
`land` reads the pull request back from the forge and enqueues it at the front after three refusals: not open, its own checks not green, or a head that is not a batch's. The two green-nesses are different questions and both are asked — the gate's green is a bench's, CI's green is the forge's on the commit the queue will take, and integration-4 went green on hulk and red on three CI legs. `--receipt-file` takes the LAST line of a file, so a caller may hand it the gate's whole output.

**The rule is asked before the forge, and a refusal and a breakdown are two words (dogfood round 5, edge 3).** The branch rule used to be reached only inside the door, after `land` had read the pull request *and* its whole check rollup — so "this is not a batch", a fact of a branch NAME, cost two round trips and arrived after the slow one. It is now asked in two steps, each as early as it can be answered: everything the invocation settles on its own — a `--receipt` that is not a `BATCH OK` line, or one whose `members=none` — is decided with the host **untouched**, and the rest the moment the forge has named the head, before the rollup read. The door asks the same exported rule again on the way through, so it still lives in one place. A refusal is **`LAND REFUSED`** at exit 1 (the verb ran and said NO); a read that failed is **`LAND ERROR`** at exit 2. Both used to print `LAND REFUSED`, which left the exit code the only thing telling apart "this does not enter the queue" from "the forge could not be read" — and a caller retries one and not the other.

`queue audit` is the other half: it lists every open pull request carrying an auto-merge and takes it off — the hand sweep that removed 27 that morning, as a verb, with every entry named and one line of counts. **The read is the default (dogfood round 5, edge 1).** *Audit* is a reading word, and the verb took `--dry-run` defaulting to false: a non-author typed the obvious `queue audit --repo <owner>/<name>` to SEE what was armed and disarmed the whole forge instead, learning it from the counts afterwards. `--apply` performs the disable; a bare invocation lists and writes nothing. `--dry-run` is kept as a spelling of the default, so a script carrying it means what it always meant, and `--apply --dry-run` together are two answers to one question, refused at exit 2. The verdict line says which one ran as a WORD — `mode=dry-run|apply`, not a boolean whose polarity a reader has to remember. It is not a lane verb and names its repository outright.
...
**Dogfood round 5, the three a non-author found on vision.** **Edge 1**: `queue audit` ACTED BY DEFAULT and its name reads read-only — `--dry-run` defaulted to false, so the obvious invocation disarmed the forge and said so afterwards; the read is now the default, `--apply` performs the disable, and the line names the mode in a word. **Edge 3**: `land` asked the branch rule only inside the door, after reading the pull request AND its whole check rollup, so the cheapest refusal in the verb cost the most expensive read — and it printed `LAND REFUSED` at exit 2 as well as exit 1, so the two answers a caller treats differently were one line. The rule is asked as early as each half can be answered, and the breakdown is `LAND ERROR`. **Edge 5**: a batch whose every member was dropped ran `build`, `vet`, `vet-windows` and `test` over a head that IS the base — two minutes proving that `dev` is green, ending in a `BATCH OK` receipt `land` then refused — and the first member's drop said `"the merge conflicts with the members ahead"` when there was nothing ahead of it. An empty batch is `BATCH FAIL … members=none reason="every member dropped"` at exit 1 before the first step, and a drop with no members ahead names the base. *A verb whose name promises a look does not write; a refusal costs the reads it needs and no more; and a gate says nothing was built before it spends two minutes proving it.*
