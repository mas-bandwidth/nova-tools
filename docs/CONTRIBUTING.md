...
`seatwrap`, `wholetree` (no doc or card spells a whole-tree `go test`; run `nova-ci local`), `silent` (no `_ = err` and no `|| true` literal on the copy model's live path), `classtests` (no merge deletes a class test file dev had). Every entry — the ten above too — is written out in
