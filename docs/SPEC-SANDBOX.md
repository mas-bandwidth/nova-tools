...
```
SANDBOX OK backend=<sandbox-exec|landlock|appcontainer> abi=<n|-> [used=<n>] read=<n> write=<n> net=<denied|nopromise> cwd=<dir> cwdb64=<base64url> ancestors=<n> cmd=<name>
...
the working directory through oneline.Field, for the operator; `cwdb64=<base64url>`
is the machine-readable receipt, a strict base64url encoding (RFC 4648 §5, no
padding) of the raw path bytes the wall applied. A reader that must match the
cwd decodes `cwdb64=` and never the readable field: oneline's escape is not
injective (a literal backslash is not escaped), so the readable spelling cannot
be reversed to the bytes. A reader that finds `cwdb64=` absent or not valid
base64url treats the wall as one that cannot name its own containment.
