...
- **New flags:** `draft --reply-to <id-or-path-or-subject>` is the bounded reply transaction — the header is the tool's, not hand-built — with `--body-file`, `--draft-dir`, `--max-body-bytes`; `draft --out <path> [--overwrite]` is the redirect in flag form (`--file` is retired on draft to avoid confusion with `send --file`); `inbox`/`wait` take `--bodies [--max-notes <n>] [--max-bytes <n>] [--after <token>]`; `wait --beat`/`--beat-lease` control the presence beat; a `To:` line may name the broadcast aliases `all` and `table`.
