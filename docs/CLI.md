...
### The bus: `nova-sprint bus`

The people doing the work talk to Rowan over Redis streams (nova-tools #3865, rowan-new specs/bus-redis.md), not markdown notes committed to a git repo. Keys, and nothing else: `bus:<to>` is a person's inbox (a STREAM of entries `from`, `to`, `kind`, `subject`, `body`, `ref`, `re`, `at` ms), read through the consumer group `<to>`; `bus:all` is the broadcast inbox, read by each person through a group of their own name, so one post reaches every reader once; `bus:sent:<from>` is the sender's outbox, the same fields plus `xid`, the inbox entry's id. A post is one FCALL (`ns_bus_post`, `internal/nsprint/fn/lua/bus.lua`): both XADDs in one step, and a person's group is made at 0 by the first post to them, so nothing sent before their first read is lost. No files, no git, no TTL, no KEYS or SCAN. People: rowan, emma, stella, johnny, glenn; a name outside the list is refused, never a new inbox. Kinds: note, request, question, answer, done, ack.

- `nova-sprint bus post --as <me> --to <who|all> --kind <k> --subject '<text>' (--body '<text>' | --body-file <f>) [--ref <ref>] [--re <xid>]` prints `POSTED to=<who> id=<xid>`.
- `nova-sprint bus read --as <me> [--n 20] [--all]` delivers up to n entries from `bus:<me>` and `bus:all`, oldest first: first any delivered before and never acked (a read that died before its ack), then new ones. Each prints as `<xid> <at ET> <from> <kind> "<subject>" [ref=<ref>] [re=<xid>] [to=all]`, then the body with every line indented two spaces; the entries are acked after they print, and the last line is `READ as=<me> n=<k>`. `--all` starts the groups at 0 and reads the history again. Entries over n stay pending and come first on the next read.
- `nova-sprint bus pending --as <me>` prints `<xid> <stream> idle=<ms> deliveries=<n>` per entry delivered and not acked, then `PENDING as=<me> n=<k>`.
- `nova-sprint bus tail --as <me>` is the blocking loop: one line per entry as it arrives, the header above plus `body="<quoted>"`, acked after it prints; SIGINT or SIGTERM ends it within a second. Rowan's session keeps `bus tail --as rowan` armed in place of the git poll.
- `nova-sprint bus ls` prints `bus:<name> len=<n> unread=<k>` for every person's inbox and `bus:all len=<n> unread=<reader>:<k>,...`, one pipeline, then `BUS inboxes=<n> unread=<k>`.
- `nova-sprint bus reply --as <me> --re <xid> (--body ... | --body-file ...) [--kind <k>] [--subject <text>] [--ref <ref>]` posts to the entry's sender with `re` set; the kind defaults to answer and the subject to `re: <subject>`.

Refused, exit 1, one `REFUSED bus <verb>: <why>; remedy: <what to do>` line, nothing written: a body over 16 KB (put it in a task record or a PR and send `--ref`); a body or subject holding a score line `SCORE who=` (scores never travel on the bus: record the read with `nova-sprint read post`, then post a note with `--ref`); an unknown name or kind; a `--re` that is not in the replier's inboxes; a store the seat cannot reach. Usage, exit 2: a missing flag, both or neither of `--body` and `--body-file`, `--to` on reply. The store is `--redis`, else `NOVA_SPRINT_REDIS`, else `NOVA_REDIS_ADDR`, with the seat from `NOVA_SPRINT_REDIS_USER`. The whole grant a seat needs for the bus is `bus.SeatRules` (`internal/nsprint/bus/bus.go`): `~bus:*`, the stream commands, and `+fcall|ns_bus_post`.
