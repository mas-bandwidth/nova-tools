...
The copy wrapper (`nova-card copy`, internal/nsprint/card/wrapper_spec.go) holds a work or fix copy's commit to the card before its end pushes anything: the diff adds or changes at least one `_test.go`; `TEST` passes at the copy's head and fails at `base-sha` with the diff's test files checked out over it (a `TEST: none <why>` card is excused from that, not from CI); then `nova-ci local --base <base-sha>` in the checkout (#4360: the unit tier CI runs for the diff), or, where the verb cannot run, `go test -json -p 2 -count=1` of the touched packages. A red ends the copy `FAILED` with the typed reason (`no-test`, `test-not-green`, `test-not-red`, `ci-red`) and one line naming the red and the remedy; no PR is opened; the rows, every `RED package=<p> test=<t>` line included, go under `## Gates` in `RESULT.md`; `wrapper.line` carries `gate=<pass|reason>` and the copy's record `evidence` = `gate=<pass|reason> red=<names|->`.
