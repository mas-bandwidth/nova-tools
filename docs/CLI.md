...
the six sets of the streams of `ws:order` (each stream's sentinel counted
like any card; parked and done cards in none of them), done is landed, left
is total minus done (a card in `review` or `merging` is left, not done), and
the ETA is now plus left over the moves to `landed` in the last hour of
`ws:log`, in Eastern (`+<n>d` when days out; `?` with no landing in the hour). Below the total, one
