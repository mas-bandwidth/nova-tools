...
The since-walk is bounded by `--max-commits` (default 500) so that a cursor left far behind costs one count and one line instead of minutes of silence. When the bound is hit the run prints `INBOX WALK bounded commits=<n> cursor=<sha> behind=more-than-<n> notes=0 remedy="raise --max-commits or close --before <instant>"` on stderr and exits 0: it says whose cursor stopped it, that nothing was read and why, and both ways out, all on the one line. If your cursor is habitually that far back, put the bound in the bus's own defaults instead of retyping it — `<bus>/.nova-bus/defaults` is `<flag>=<value>` lines and takes **any** flag of the verb, not just `receipt-max-words`, with the command line always winning.
