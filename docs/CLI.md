...
- The stream sentinel (#4318): every stream has one sentinel card, `<slug>:sentinel` (the stream name lower-cased, runs of other characters one `-`), created in the stream's waiting set when the stream is registered (its first push, or `stream order`). It is the stream's stop: it is never dealt, and `task land --id <slug>:sentinel --sha <merge sha>` lands it only when every other card of the stream is landed or done (refused by name until then). A stream that must wait for another whole stream puts `DEPENDS-ON <slug>:sentinel` on its first card; the waiting resolver treats it like any card edge, and there is no second kind of dependency.
