...
**Draft 2, 2026-09-13.** No reader has approved this document. It is normative in
...
> Draft 2 folds the two cold reads of draft 1 (`f6c8bd95`). What changed, in one list:
> the scope is a declared fact and not only a flag; every binary this tool executes is
> declared; the canary runs inside the real nesting; `harness.credential`,
> `harness.config`, `process.holder` and the required-by table exist, so the rules that
> lean on them can be written; the exit boundary is one law; rules 23 and 24 are numbered;
> and the rules that contradicted each other — rule 5 against rule 18, rule 21 against the
> line it starts — have one survivor each.
...
```json
{
  ...
  "scope": "account | home",
  "role":  "<a label; this tool interprets only \"keeper\">",
  "tools": {
  ...
  "sandbox": {
    "read": ["/absolute/dir", "..."], "write": ["/absolute/dir", "..."],
...
|---|---|
| `name`, `box`, `scope`, `role`, `account`, `secrets`, `sandbox`, `tools.secrets`, `tools.sandbox` | always |
...
| `secrets.only` | with `secrets`; empty exactly when `harness.credential` is `none` |
...
or its work needs no provider, and `secrets.only` is empty — **which is legal**, and is the
first case ideas#766 asks to prove.
...
1. **One declaration, one line, from a flag, in git.** No default path, no search of the
   ...
   `harness.bin`. A file declaring more than one line is a refusal: the fleet is
...
3. **It never runs a privileged command, it never creates an account, and it never
   ...
   doing it. The ACCOUNT scope likewise **measures** the unix user and, where it is absent,
...
12. **READY is a measurement, and it is the only thing that makes a line up.** After the
    ...
    **And a process that exits before READY ends the wait at once**, rather than waiting
    out a deadline for a probe that will never answer: exit `125` is the wrapper's own
    refusal ([SPEC-SECRETS.md](SPEC-SECRETS.md) and [SPEC-SANDBOX.md](SPEC-SANDBOX.md)
    both refuse at 125 before the command starts), reported as `FAIL` reason `refused`
    carrying the child's first stderr line, bounded — `SANDBOX REFUSED reason=home_outside`
    is the truth, and a readiness timeout would have been a lie; any other early exit is
    `FAIL` reason `died` carrying its status. (2026-09-09, a harness whose background poll
...
14. **This tool never opens a key file, and secrets arrive by exactly one route.** The
    ...
    **`process.env` is a git-tracked map and is therefore not a place for a credential**:
    ...
    a secret in git, which is the thing the store exists to prevent.
...
15. **The nesting order is load-bearing and fixed: secrets outside, wall inside, harness
    ...
    in **one function with no flag that reaches it** — the same function builds the canary
    (rule 11) and the start (step 11), which is why the canary cannot drift from the thing
    it is a rehearsal for.
...
17. **The wall is not optional and is proved before the first start.** `up` runs
    ...
    (rule 13); a path in both lists is a refusal naming both flags; and `sandbox.listen`
...
18. **One process per line per box, and its data home is its own.** Before starting
    ...
    **every step but `ready` print `SKIP`** and the run a repair, never a second start —
    readiness is measured again on a repair as on any other run, because it is never a
    memory (rules 5 and 12, which this rule obeys rather than re-decides).
    `sandbox.home_env` is the line's own directory and appears in no other declaration
    this tool has state for — which is why the state file records each line's `home_env`
    (below) and not only its pid. (2026-09-10: six concurrent workers sharing one harness
...
19. **One keeper, and this tool holds the half it can see.** Glenn, ideas#766: *"if there
    ...
    belongs to the tool that can see two boxes: nova-admin. **`"keeper"` is the only role
    this tool interprets**; every other value is a label it prints and nothing else, because
    what a line's role means on a given estate is that estate's practice and not a tool's
    business.
...
21. **Bounded output, by design, at the largest plausible state.** One line per event.
    ...
    **Every subprocess this tool waits for** — a version read, a canary, a probe, a
    ...
    line this tool exists to start. `logs` reads the file afterwards, bounded, as any
    reader would.
...
22. **Every refusal says what the input wants and names the next command; one run reports
    ...
    that failed, a probe that never answered: a step ran and said NO, and that is exit 1.
...
the pid it started, **the process group id it made** (rule 13 and `down`), the
declaration's sha256, the box label, the harness identity it proved,
**`sandbox.home_env`**, **`process.log`**, and the stamp. The last two are what let rule
18 refuse a second line onto one data home and let `logs` answer for a line whose
declaration moved. It is the caller's path, never `$HOME`, never a temp directory, and a
...
vanished between the existence test and the stat lost three of twenty writes.
...
and 1 when something would have to change. **Two exemptions, named rather than
discovered**: step 5 runs `nova-sandbox probe`, which is a process and which writes inside
the write set — proving the wall is the one measurement that cannot be taken by looking —
...
| 5 | `wall` | `nova-sandbox probe` green under exactly the declared lists, with `secrets.key` as the secret it must fail to read | never — the probe is the named exemption to `--dry-run`'s "no process" |
...
| 11 | `start` + `ready` | the state file's pid, if any, is alive and is this line's | builds the same nesting with `start_argv` innermost, starts it **in a process group of its own**, then measures readiness (rule 12) |
...
Stops the process the state file names — a term signal to **the process group**, then the
declared timeout, then a kill to the same group — pushes `home.path` to its own remote
(rule 20), reports `dirty=<n>` for uncommitted work it did not touch, and clears the state
file's pid under the same lock. **The group is this tool's own making, at `up`**
(`setpgid` on the outermost child), because nova-sandbox deliberately makes none —
*"the wrapped tree stays in the caller's group, and the caller owns pgid and reaping"*
([SPEC-SANDBOX.md](SPEC-SANDBOX.md) rule 12, darwin) — and this tool is the caller. A
signal to the pid alone would kill the waiting wrapper and orphan the harness, which is
2026-09-10's nineteen shells with a better name. A line already down is exit 0 and says
...
A child's `125` is not this tool's exit code: it is a measurement this tool reports as
`FAIL` reason `refused`, and this tool exits 1 (rule 12).
...
`stash`, no `clean`, no `--force`, no `push -f`, no `security find-generic-password`, and no
...
1. `TestOneDeclarationFromAFlag` — no `--file` is exit 2 printing `refusing to guess`; a
   ...
   from `box`, and `--scope` differing from `scope` are each exit 2 naming both; no child
   is ever launched by a bare name (every exec site is asserted to receive an absolute
   path from `tools` or `harness.bin`).
2. `TestEveryPathIsAbsoluteAndDeclared` — a relative path in any field is exit 2 with the
   ...
   path is exit 2 naming both fields; after a run, the only directories created are
...
3. `TestItNeverRunsAPrivilegedCommand` — the source tripwire, plus a behavioral half: a
   ...
   `PATH` count zero runs in every verb.
...
6. `TestTheHomeIsNeverRewritten` — absent clones at the declared branch; behind
   ...
   Mutation: a `reset --hard` on the divergent case, which must turn every one of the six
...
12. `TestReadyIsAMeasurement` — a fake line whose process is alive and whose probe never
    ...
    counts as a NO; a child that exits `125` before READY is `FAIL` reason `refused`
    carrying its first stderr line within one interval, and any other early exit is `FAIL`
    reason `died` — both asserted to be faster than `ready.deadline`; a declaration with no
...
14. `TestNoKeyFileIsEverOpened` — a distinctive 40-byte fixture value in the throwaway
    ...
    **the proof of non-opening is the process's own open-file list, which never contains
    ...
    is exit 2 naming it; a mutation printing `len(value)` turns it red; the source tripwire
    has no `source`, no `eval`, no Keychain call and no crypto dependency.
15. `TestTheNestingOrderIsFixed` — the built argv is exactly
    `<tools.secrets> exec … -- <tools.sandbox> … -- <innermost>`, asserted on the argv the
    **fake** children see, for the canary and for the start, from the one builder; the
    store, the key file and `sops` appear in **neither** sandbox list; the reverse nesting
    is asserted to **fail** and is unreachable from any flag. The assertions run against
...
16. `TestAHarnessThatCannotTakeItsKeyByEnvironmentIsRefused` — `harness.credential: "file"`
    ...
    `credential: "none"` with an empty `secrets.only` runs to `READY` — the keyless line,
    which draft 1 would have refused; `credential: "env"` with an empty `only`, and
    `credential: "none"` with a non-empty one, are each exit 2 naming the condition;
...
17. `TestTheWallIsNotOptional` — a failing probe, and a box with no backend, each refuse
    ...
    sentences; a path in both lists is a refusal naming both; `listen: true` with
...
18. `TestOneProcessPerLinePerBox` — with a live pid in the state file, a second `up`
    starts nothing and every step **but `ready`** prints `SKIP`, `ready` being measured
    again (the assertion that rules 5, 12 and 18 agree); a dead pid is reaped and the line
    starts; two declarations sharing one `sandbox.home_env` are exit 2 naming both, read
    from the state file's recorded `home_env` and not from a second declaration; concurrent
    `up` runs over one state file leave it parseable and one of them exits 2 naming the
    holder's pid, never a 0-byte file.
19. `TestOneKeeper` — `role: "keeper"` with a live keeper process for that line is exit 1
    ...
    other step is green — the 2026-09-07 case with the bigger blast radius; two other role
    labels, including one this repository has never used, are unaffected and are printed
    verbatim; there is no `move`, `handover` or `promote` verb, asserted on the verb table.
...
21. `TestOutputIsBoundedAtTheLargestPlausibleState` — a 200 MB log gives `logs` at most
    ...
    because draft 1's cap would have killed the line it started.
22. `TestEveryRefusalNamesItsRemedyAndItsExit` — every refusal in the package lives in one
    ...
    declaration fact and 1 for every box fact**; removing one, or flipping one's code,
    turns it red. One run with six independent problems names all six, sorted, in one
...
125 refusal); [SPEC-SANDBOX.md](SPEC-SANDBOX.md) (rules 1, 7, 9, 10, 11, 12, 13, the
launcher checklist items 3 and 5, the exit table's `bad_net` and `home_outside`, and
