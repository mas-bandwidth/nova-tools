...
| `from-<me>/BEAT` | one reader's | one line, an RFC 3339 UTC stamp, the cursor sha, and a `until=<stamp>` lease, rewritten on `wait` entry, every poll tick, and exit so a waiting line's cursor that does not move still records that the line is alive — and the lease keeps a coordinator cycle between two waits reading awake |
