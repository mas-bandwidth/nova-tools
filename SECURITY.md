...
Which verbs are walls, which assert nothing, what each check says NO to, and what
every exit code means are all specified there, per tool, normatively. Read it for
the rule and treat this page as the shape. **Where the two disagree, `SPEC.md`
governs, and the disagreement is itself worth reporting** — four drafts of this
page contradicted it in ways a careful reading of this page alone would never
have caught.

So the classes, stated so they do not depend on an enumeration that can rot:

**A check that passes a case its own "says NO when" list covers.** These tools
exist to refuse, and a green from one is a thing you will act on. A wall that
passes what it was built to stop is worse than no wall, because you stop looking.
This is the most valuable finding you can bring us.
...
**Losing a fuse.** Specifically, anything that reads a box as clear when that box
records a lockdown, or reads an unreadable box as clear at all — `SPEC.md` is
explicit that absent and unreadable are different answers and that collapsing
them is the fail-open the package exists to prevent.
...
feature. Disagreeing with a judgment from the advisory instrument, which flags so
that you decide. And anything `SPEC.md` declares: it names its permanent misses,
its blind spots, its accepted collapses and the lists it says are not exhaustive,
and each of those is the spec working rather than a hole. **If a declared limit
looks wrong to you, that is a finding about the spec, and we want it as one.**
...
the wire would answer first. So here is an anchor you can check without trusting
mail: **we will never propose a channel or publish a key by mail alone. Anything
we offer will also appear in a commit to this file**, so read
[its history](https://github.com/mas-bandwidth/nova-tools/commits/main/SECURITY.md)
before you use a channel someone has offered you. That first message is itself a signal that an
