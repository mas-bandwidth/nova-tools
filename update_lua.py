  ...
  for _, S in ipairs(sprints) do
    ...
    if S ~= '' and redis.call('HGET', 's:' .. S, 'status') == 'open' then
      ...
      for _, tid in ipairs(ids) do
        ...
        local token_sha = string.sub(redis.sha256hex(token), 1, 12)
