...
# (3c) sbcl version and path -- the pin is 2.5.8 under ~/sdk (nova-tools#2053).
NOVA_SBCL="${NOVA_SBCL:-2.5.8}"
if command -v sbcl >/dev/null 2>&1; then
  sbcl_ver="$(sbcl --version 2>&1 || true)"
  case "$sbcl_ver" in
    *"$NOVA_SBCL"*) ;;
    *) drift "sbcl version [$sbcl_ver] want $NOVA_SBCL" ;;
  esac
  sbcl_path="$(command -v sbcl)"
  case "$sbcl_path" in
    "$HOME_DIR/sdk/"*) ;;
    *) drift "sbcl path [$sbcl_path] not under ~/sdk (want ~/sdk/sbcl-$NOVA_SBCL)" ;;
  esac
fi

# (3d) sshd MaxSessions / MaxStartups -- part of the bench standard (#2018).
NOVA_SSHD_MAX_SESSIONS="${NOVA_SSHD_MAX_SESSIONS:-200}"
NOVA_SSHD_MAX_STARTUPS="${NOVA_SSHD_MAX_STARTUPS:-300:30:600}"
if [ "$OS" = "Linux" ] && [ -f /etc/ssh/sshd_config ]; then
  got_sessions="$(grep -i '^MaxSessions' /etc/ssh/sshd_config 2>/dev/null | awk '{print $2}' || true)"
  if [ -n "$got_sessions" ] && [ "$got_sessions" != "$NOVA_SSHD_MAX_SESSIONS" ]; then
    drift "sshd MaxSessions=$got_sessions want=$NOVA_SSHD_MAX_SESSIONS"
  fi
  got_startups="$(grep -i '^MaxStartups' /etc/ssh/sshd_config 2>/dev/null | awk '{print $2}' || true)"
  if [ -n "$got_startups" ] && [ "$got_startups" != "$NOVA_SSHD_MAX_STARTUPS" ]; then
    drift "sshd MaxStartups=$got_startups want=$NOVA_SSHD_MAX_STARTUPS"
  fi
fi

# (3e) rungs -- pro must be available on every bench (nova-tools#2053).
...
if [ "$pro_rung" = "0" ]; then
  drift "pro rung missing (no ~/nova-bench/rungs/pro or ~/nova-bench/pro)"
fi

# (3f) bench-standard.sh itself installed on the bench.
if [ ! -f "$HOME_DIR/nova-bench/tools/bench-standard.sh" ]; then
  drift "bench-standard.sh not installed at ~/nova-bench/tools/bench-standard.sh"
fi

# (3g) extra Go toolchains -- only the pinned version under ~/sdk.
if [ -n "$NOVA_GO" ] && [ -d "$HOME_DIR/sdk" ]; then
  for gd in "$HOME_DIR"/sdk/go*/; do
    [ -d "$gd" ] || continue
    gname="$(basename "$gd")"
    case "$gname" in
      "$NOVA_GO") ;;
      go*) drift "extra Go toolchain $gname under ~/sdk (want only $NOVA_GO)" ;;
    esac
  done
fi

# (3h) layout -- sqlite3 must resolve from ~/sdk, not /usr/bin.
if command -v sqlite3 >/dev/null 2>&1; then
  sqlite3_loc="$(command -v sqlite3)"
  case "$sqlite3_loc" in
    "$HOME_DIR/sdk/"*) ;;
    *) drift "sqlite3 at $sqlite3_loc not under ~/sdk" ;;
  esac
fi

# (3i) bench user -- the standard user is nova.
NOVA_BENCH_USER="${NOVA_BENCH_USER:-nova}"
got_user="$(whoami 2>/dev/null || echo unknown)"
if [ "$got_user" != "$NOVA_BENCH_USER" ]; then
  drift "bench user=$got_user want=$NOVA_BENCH_USER"
fi

# (3j) slot share -- every bench must declare its share.
NOVA_SLOT_SHARE="${NOVA_SLOT_SHARE:-}"
if [ -z "$NOVA_SLOT_SHARE" ]; then
  drift "NOVA_SLOT_SHARE unset (set to the bench slot share in GB)"
fi

# (3k) coordinator -- if this bench is a coordinator, declare it.
NOVA_COORDINATOR="${NOVA_COORDINATOR:-0}"
