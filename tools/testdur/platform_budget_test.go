...
func TestAnUnreadableFactorIsRefused(t *testing.T) {
	for _, bad := range []string{
		...
		"## Bench: b, darwin/arm64, budget-factor: NaN",
		"## Bench: b, darwin/arm64, budget-factor: +Inf",
		"## Bench: b, darwin/arm64, budget-factor: -Inf",
		"## Bench: b, darwin/arm64, budget-factor: Inf",
		"## Bench: b, darwin/arm64, budget-factor: 1e308",
	} {
...
// TestBudgetBenchFactorDoesNotExpandFixedSixtyBaseline verifies that even if the
// [budget] bench heading states a budget-factor > 1 (e.g. 2.0), the baseline
// enforced for the fallback (unknown platform) and on the budget bench itself
// remains strictly the fixed sixty-second ceiling.
func TestBudgetBenchFactorDoesNotExpandFixedSixtyBaseline(t *testing.T) {
	const record = `## Bench: Space, linux/amd64, 16 cores [budget], budget-factor: 2.0
...
`
	...
	// 1. Unknown platform fallback must remain 60.0s, NOT 120.0s
	got, bench, err := budgetFor(benches, "unknown/platform")
	if err != nil {
		t.Fatalf("budgetFor(unknown/platform): %v", err)
	}
	if got != 60.0 {
		t.Errorf("budgetFor(unknown/platform) = %.1fs, want strictly 60.0s fixed baseline", got)
	}
	if !strings.Contains(bench.heading, "Space") {
		t.Errorf("fallback bench = %q, want Space", bench.heading)
	}

	// 2. Budget bench native platform (linux/amd64) must also remain 60.0s
	got, bench, err = budgetFor(benches, "linux/amd64")
	if err != nil {
		t.Fatalf("budgetFor(linux/amd64): %v", err)
	}
	if got != 60.0 {
		t.Errorf("budgetFor(linux/amd64) = %.1fs, want strictly 60.0s fixed baseline", got)
	}

	// 3. Non-budget platform (darwin/arm64) still uses its platform factor (60 * 2.2 = 132.0s)
	got, _, err = budgetFor(benches, "darwin/arm64")
	if err != nil {
		t.Fatalf("budgetFor(darwin/arm64): %v", err)
	}
	if got != 132.0 {
		t.Errorf("budgetFor(darwin/arm64) = %.1fs, want 132.0s", got)
	}
}
