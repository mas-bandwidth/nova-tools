...
# bench-hygiene.sh [--dry-run]: with --dry-run it prints WOULD lines and deletes nothing.
# HYGIENE <host> slots=<n> reaped=<n> jobs-deleted=<n> slots-deleted=<n> cache=<kept|dropped>(<size>) free <a> -> <b>
DRY=0; [ "${1:-}" = "--dry-run" ] && DRY=1
rm_() { if [ $DRY = 1 ]; then echo "WOULD rm -rf $*"; else rm -rf "$@" 2>/dev/null; fi; }
...
for s in $(for r in $ROOTS; do ls -d "$r"/*/ 2>/dev/null; done); do s=${s%/}; [ -d "$s" ] || continue; slots=$((slots+1)); live=0; newest=0
  for j in "$s"/jobs/*/; do j=${j%/}; [ -d "$j" ] || continue
    ...
    # authoritative liveness: any process whose command line or cwd names the job dir (Stella's read, 2026-09-17)
    pgrep -f -- "$j" >/dev/null 2>&1 && live=1
  done
  [ $live = 0 ] && for pid in $(ls /proc 2>/dev/null | grep -E '^[0-9]+$'); do case "$(readlink /proc/$pid/cwd 2>/dev/null)" in "$s"*) live=1; break;; esac; done
  [ $live = 1 ] && continue
  # finished or dead: the slot's data home, tmp and scratch go at once
  { [ -d "$s/data" ] || [ -d "$s/tmp" ]; } && { rm_ "$s/data" "$s/tmp" "$s"/jobs/*/scratch "$s"/jobs/*/.nova-sandbox-tmp "$s"/jobs/*/repo/scratch; reaped=$((reaped+1)); }
  # read, then delete: a harvested job goes whole; a finished job nobody read goes after six hours; an empty slot goes
  for j in "$s"/jobs/*/; do j=${j%/}; [ -d "$j" ] || continue
    if [ -f "$j/.harvested" ] || { [ $newest -gt 0 ] && [ $(( (now - newest) / 3600 )) -ge 6 ]; }; then rm_ "$j"; jobs=$((jobs+1)); fi
  done
  if [ -z "$(ls -A "$s/jobs" 2>/dev/null)" ]; then rm_ "$s"; dropped=$((dropped+1)); fi
done
# runner scratch older than a day; the checked-out trees stay (the runner reuses them)
for w in "$HOME"/runner-nova-tools-*/_work/_temp; do [ -d "$w" ] && find "$w" -mindepth 1 -maxdepth 1 -mmin +1440 -exec rm -rf {} + 2>/dev/null; done  # (dry-run still prunes runner _temp; it is the runner's own scratch)
...
if [ "$free_g" -lt 25 ] || [ "$size_g" -gt 20 ]; then rm_ "$HOME/.cache/go-build"; cache=dropped; fi
