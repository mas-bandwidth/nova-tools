...
v_run() {
  ...
  for r in "$ROOT1" "$ROOT2"; do
    for s in "$r"/*/; do
      ...
      for j in "$s"/jobs/*/; do
        ...
        m=$(stat -c %Y "$j/harness-output.log" 2>/dev/null) || continue
      ...
      for j in "$s"/jobs/*/; do
        j=${j%/}; [ -d "$j" ] || continue
        if [ -f "$j/harness-output.log" ]; then m=$(stat -c %Y "$j/harness-output.log"); [ "$m" -gt "$newest_j" ] && newest_j=$m; [ ! -f "$j/RESULT.md" ] && [ $(( (now - m) / 60 )) -lt 15 ] && live=1; fi
  ...
  size_g=$(du -sBG "$HOME/.cache/go-build" 2>/dev/null | awk '{gsub("G","",$1); print $1}'); size_g=${size_g:-0}
