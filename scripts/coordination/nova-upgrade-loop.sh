...
while [ "$(date +%s)" -lt "$end" ]; do
  ...
  if [ -n "$now" ] && [ "$now" != "$last" ]; then
    ...
    if [ -n "$changed" ] || [ -z "$last" ]; then
      ...
      # Build once, cache by sha, install on every Linux bench, verify (fleet-install-tools.sh). The line this replaces built on
      # space into ~/nova-bench/bin, a directory nothing used, and installed nowhere: the benches ran six-hour-old tools (pit stop 2026-09-17).
      perl -e 'alarm shift; exec @ARGV' 900 "$HOME/rowan-working/bin/fleet-install-tools.sh" "$now" 2>/dev/null | sed 's/^/FLEET /' >> "$ROOT/upgrade.log" || echo "FLEET INSTALL FAILED OR TIMED OUT sha=${now:0:12}" >> "$ROOT/upgrade.log"
