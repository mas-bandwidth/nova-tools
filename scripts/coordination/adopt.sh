...
run() { perl -e 'alarm shift; exec @ARGV' "$@" 2>&1; }
# 1 versions agree
...
probe zen opencode/gemini-3.8-flash 5 # Zen (metered): one cheap route for independent second reads on another family (Glenn 2026-09-16: use Zen)
...
label=local; n=7; safe_rm "$R/9$n/jobs/$label"; mkdir -p "$R/9$n/jobs/$label"; (cd "$R/9$n/jobs/$label" && git init -q .); l=$(run 420 "$HOME/.local/bin/nova-swarm" native --harness /Users/Shared/nova-swarm-stella-20260913/harness-v1.18.20/opencode --model ollama/north-mini-code-32k --label local --card "$C/probe-local.md" --slot "$R/9$n" --root "$R" --deadline 360s --auth "$HOME/.local/share/opencode/auth.json" --config "$HOME/.config/opencode/opencode.json" | tail -1); r=$(sed -n "2p" "$R/9$n/jobs/$label/RESULT.md" 2>/dev/null); case "$r" in "count: "[0-9]*) ok route-local ollama/north-mini-code-32k;; *) printf 'ADOPT NOTE route-local ollama/north-mini-code-32k no result (informational; issue #591)\n' >> "$OUT";; esac
# 5 snapshot then report
run 60 nova-version snapshot --bin "$HOME/.local/bin" --out "$S/manifest.tsv" --owner Rowan >/dev/null; l=$(run 60 nova-version report --file "$S/manifest.tsv" | tail -1); case "$l" in "REPORT OK"*) ok snapshot-report "${l:0:60}";; *) bad snapshot-report "${l:0:120}";; esac
...
want=$(grep -m1 '^go ' "$HOME/rowan-working/nova-tools/go.mod" | awk '{print $2}'); got=$(perl -e 'alarm 25; exec @ARGV' ssh -o BatchMode=yes space 'HOME=/home/ubuntu/nova-bench/swarm-root/101/data GOTOOLCHAIN=local go version 2>&1' | grep -o 'go[0-9][0-9.]*' | head -1); case "$got" in go$want*|go${want}.*) ok space-go "$got satisfies go $want in a card HOME";; *) bad space-go "space card sees ${got:-nothing}, go.mod wants $want";; esac
